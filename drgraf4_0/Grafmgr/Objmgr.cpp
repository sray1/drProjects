...
UINT CObjectMgr::GetLastInputNo(int nObjectType)
{
	...
	//////////////////////////////////////////////////
	int num = (nInputIndex>=0?(m_nLastInputNo[nInputIndex]):-1);
	return	num;
}
...
int CObjectMgr::DeleteFromDataBase(int nObjIndex, int nObjType)
{
	...
	CDrGrafDoc* pDoc 			= ((CDrGrafApp*)AfxGetApp())->GetDocument();
	CDListMgr*	pDListMgr 		= GetObjectList(nObjType);
	CDListMgr*	pLabelList		= GetObjectList(LABEL); 
	CDrLabel*	pLabel; 
	//////////////////////////////////////////////////////////////////////////////// DataView/RowList
	int nRowIndex;
	...
	///////////////////////////////////////// Remove Label,if Any
	if(!(pObject->GetLabelList()->IsEmpty()))//
	{
		
	 	POSITION pos = pObject->GetLabelList()->GetFirstObjectPos();
		int i;
		for (i=0;pos !=NULL ;i++)
		{
			///////////////////////////////// private List
			pLabel = (CDrLabel*)pObject->GetLabelList()
												->GetNextObject( pos);
			///////////////////////////////// DataBaseList 
			int indexLabel = pLabelList->GetObjectIndex(pLabel);
			if(indexLabel>=0)
				pLabelList->RemoveObject(indexLabel);
		}
	}
	////////////////////////////////////////////////////////////// Now delete
...
int CObjectMgr::ShowObject_On_Off(CDrObject* pObject,UINT GenObj,BOOL bOn)
{
	int nResult = 0;
	///////////////////////
	CDListMgr* pList;
	///////////////////
	SetActiveObjectType(GenObj);
	////////////////////////////////////////////////////////////////////////////// 
	pObject->SetShow(bOn);	
	int nActiveIndex;
	/////////////////
	...
	/////////////////////////////////////////////////
	pList = GetObjectList(GenObj);
	if(!pList)
		return nResult;
	nActiveIndex = pList->GetObjectIndex(GenObj, ID);
	if(nActiveIndex<0)
		return nResult;
	///////////////////
	SetActiveObjectIndex(nActiveIndex);	// set Active Index									
	SetActiveObjectType(GenObj);		    // set Active Type									
	UpdateAllViewsWithActiveIndex(NULL,nActiveIndex,GenObj);
	////////////////////////////////////
	pList = pObject->GetLabelList();
	if(pList)
		nResult = ShowLabel_On_Off(pList,ID,bOn);
	///////////////
	return nResult;
}

int CObjectMgr::ShowLabel_On_Off(CDListMgr* pList,CString ID,BOOL bOn)
{
	int nResult = 0;
	int nActiveIndex;
	////////////////////////////////////////////////////////////////////////////// 
	if(pList)
	{
		nActiveIndex = pList->GetObjectIndex(LABEL, ID);
		if(nActiveIndex>=0)
		{
			CDrLabel* pLabel = (CDrLabel*)pList->GetObject(nActiveIndex);
			pLabel->SetShow(bOn);
			/////////////
			SetActiveObjectIndex(nActiveIndex);	// set Active Index									
			SetActiveObjectType(LABEL);		    // set Active Type									
			UpdateAllViewsWithActiveIndex(NULL,nActiveIndex,LABEL);
		}
	}		
	///////////////////////////////
	return nResult;
...
int CObjectMgr::SetObjInfoID(CString& ObjID,UINT nObjType)		
{
	char buf[50];
	/////////////////////////////
	switch(nObjType)
	...
	/////////////////////////////
//	BumpLastInputNo(nObjType);					// bump it
//    int nLastNum	= GetLastInputNo(nObjType);		// internal
    int nLastNum	= GetLastInputNo(nObjType);		// internal
	BumpLastInputNo(nObjType);					// bump it
	//////////////////////////
...
int CObjectMgr::SetNodeInfoID(CString& ObjID,BOOL bCNode,UINT uType)		
{
	/////////////////////////////////////////////////////////
	CDrGrafDoc* pDoc 		= ((CDrGrafApp*)AfxGetApp())->GetDocument(); 
	...
	if(bCNode)
	{
		if(uType == MCNODE)
		{
			pObjectMgr->SetElemInfoID(ObjID,uType);		
			ObjID = "MC_"+ObjID;
		}
		else
		if(uType == CNODE)
		{
			SetObjInfoID(ObjID,uType);		
			ObjID = "CN_"+ObjID;
		}
	}
	else
...
int CObjectMgr::SetNodeInfoID(CString& ObjID,BOOL bCNode,BOOL bMeshMode)		
{
	/////////////////////////////////////////////////////////
	CDrGrafDoc* pDoc 		= ((CDrGrafApp*)AfxGetApp())->GetDocument(); 
	CMsObjMgr* pObjectMgr 	= pDoc->GetMsObjMgr();
	...
	if(bCNode)
	{
		if(bMeshMode) // MC_NODE
		{
			pObjectMgr->SetElemInfoID(ObjID,MCNODE);		
			ObjID = "MC_"+ObjID;
		}
		else
		{
			SetObjInfoID(ObjID,CNODE);		
			ObjID = "CN_"+ObjID;
		}
	}
	else
	{
		if(bMeshMode) // MNODE
			pObjectMgr->SetElemInfoID(ObjID,MNODE);		
		else
			SetObjInfoID(ObjID,NODE);
	}
	/////////
	return 0;
}

CDListMgr* CObjectMgr::GetNodeList_NodeContext(UINT uType)						// NodeContext		
{
	/////////////
	switch(uType)
	{
		case MCNODE:	return GetObjectList(MCNODE);
		case CNODE:		return GetObjectList(CNODE);
		case MNODE:		return GetObjectList(MNODE);
		case INODE:	//fall thro'
		case NODE:		return GetObjectList(NODE);
		default:		return (CDListMgr*)NULL;
	}
}

CDListMgr* CObjectMgr::GetNodeList_OtherContext(BOOL bCNode,BOOL bMeshMode)		// OtherContext
{
	// bMode	//TRUE = Mesh Mode /FALSE = Generator Mode
	/////////////
	if(bCNode)
	{
		if(bMeshMode) // MC_NODE
			return GetObjectList(MCNODE);
		else
			return GetObjectList(CNODE);
	}
	else
	{
		if(bMeshMode) // MNODE
			return GetObjectList(MNODE);
		else
			return GetObjectList(NODE);
	}
}
/////////////////////////////////////////////////////////////////////////////////////////		
