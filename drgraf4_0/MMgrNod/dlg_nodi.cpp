...
BOOL CDlg_NodI::OnInitDialog()
{
	...
	//////////////////////// Turn off Multiple for NOW   todo LATER
	GetDlgItem(IDC_MULTIPLE)->EnableWindow(FALSE);
   ////////////////////
...
void CDlg_NodI::OnIncrement()
{
	...
	if(m_bIncrement)
	{
		...
		GetDlgItem(IDC_COORDHEAD)->SetWindowText("Coord");
	}
	else
	{
		m_bDirection	= TRUE;
		GetDlgItem(IDC_OTHER)->SetWindowText("Length");
		GetDlgItem(IDC_COORDHEAD)->SetWindowText("value");
	}
	///////////////////////
//	if(m_pWnd !=NULL)
...
//			m_pWnd->SendMessage(ID_UPDATE_NODIN,IDC_ADVANCED);
	...
	///
	pObjectMgr->SetNodeInfoID(m_ObjID,m_bnid_C,FALSE);
	/////////////////
	UpdateData(FALSE);
	/////////////////
}
