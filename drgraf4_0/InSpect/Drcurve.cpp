...
//	m_pCon_BZ			= NULL;	// stored in m_CNodeList
	...
	m_FromID_S			= "";		// also used:AnchorID for Sweep/Duct
	m_ToID_S			= "";
...
/*
	...
	//////////////////////////////////////////
	j = sprintf(buf,"CDrCurve::ReadyToDelete()\n");
	j += sprintf(buf + j,"\t# Current CurveID = %s\n",GetObjectID());
	j += sprintf(buf + j,"\nNodeList:\n\t# of CNodes = %d\n",pNodeList->GetCount());
	////////////
#endif
*/
...
#ifdef _DEBUG
	CString ID;
	ID = pObject->GetObjectID();
	////
	j += sprintf( buf + j, "\n\tCNodeID Just Removed = %s\n",ID );
	j += sprintf(buf + j,"\nCurveList:\n\t# of Curves = %d\n",pCurveList->GetCount());
	////////////////////
	AfxMessageBox(*str); // DLL=2 / SubRoutine=1
	////////////
	j += sprintf( buf + j, "Curves are:\n");
	////////////////////////////////////
...
//	m_pCon_BZ			= pAdd->m_pCon_BZ;	// stored in m_CNodeList
	//////////////////////////////////////////////////////////////////// CONICS
	m_FromID_S			= pAdd->m_FromID_S;		// also used:AnchorID for Sweep/Duct
	m_ToID_S			= pAdd->m_ToID_S;
...
//	pAdd->m_pCon_BZ			= m_pCon_BZ;	// stored in m_CNodeList
	//////////////////////////////////////////////////////////////////// CONICS
	pAdd->m_FromID_S		= m_FromID_S;		// also used:AnchorID for Sweep/Duct
	pAdd->m_ToID_S			= m_ToID_S;
