	 ...
	 CCreateContext* pContext)
{
	...
	// add the first splitter pane - the tree view in column 0
	if (!m_wndSplitter.CreateView(0, 0,
		RUNTIME_CLASS(CDrTreeVu), CSize(200, 50), pContext))
	{
		TRACE0("Failed to create first pane\n");
		return FALSE;
	}

	CDrTreeVu *pView = (CDrTreeVu*)(m_wndSplitter.GetPane(0,0));
//	pView->Init();	// done in DrTreeView::OnCreate
	...
	if (!m_wndSplitter2.CreateView(0, 0,
		RUNTIME_CLASS(CDataVu), CSize(0, cyText), pContext))
	{
		TRACE0("Failed to create second pane\n");
		return FALSE;
	}
	if (!m_wndSplitter2.CreateView(1, 0,
		RUNTIME_CLASS(CDrListVu), CSize(0, 0), pContext))
	{
