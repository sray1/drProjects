...
/*				
	sprintf(str.GetBuffer(150),"CMP_Post::GoDoIt\nGenerateMeshNodes:\nTakes %8.0f seconds.", elapsed_time );
	AfxMessageBox(str);
	str.ReleaseBuffer();
	//////////////////////	
	time( &start );
...
//			pAddNode = PostNode(Nid,LocalPos,FALSE);
			pAddNode = PostNode(Nid,LocalPos,FALSE,TRUE);	// 10/23/96
...
//			vCrossProduct((Vector*)&a,(Vector*)&b,(Vector*)&v0);
		...
		case SU_TWICE:
		    ...
		    sprintf(buf,"%c",'c');
		    PolyID += buf; 
			pNode = PostNode(PolyID,wcIx,FALSE,TRUE);
...
CDrNode* CMP_Post::PostNode(CString& Nid,WORLD Pt,BOOL bCNode)
{
	...
	pNode = CreateNode(Nid,pLabel,Pt,bCNode,TRUE,nNodeIndex,nLabelIndex);	// TRUE = Internally Generated
	//////////////////////////////////////////////
	return pNode;
}
*/			
CDrFE2D* CMP_Post::MakePoly_3(CString& PolyID,int ix0,int ix1,int ix2)
{
	...
	ix[2] = ix2;
//	ix[3] = ix2;
    ////////////////////////////////////////////////// Add FE2D to FE2DList 
	int nActiveIndex;
    CDrFE2D* pPoly 	= (CDrFE2D*)(pObjectMgr->AddToDataBase(nActiveIndex,FE2D));
    //////////////////////////////////////////////////  NO show
	pPoly->SetShow(FALSE);
	/////////////////////////////////////////////////// ID
