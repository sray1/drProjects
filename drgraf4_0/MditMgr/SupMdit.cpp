...
////////////////////////////////////
IMPLEMENT_SERIAL(CSupMdit,CSolMdit,1)
/////////////////////////////////////////////////////////////////////////////
CSupMdit::CSupMdit()
{
...
void CSupMdit:: ElementLeftDown_SUP()
{	
...
int CSupMdit::ShowNodeLinSuppToEdit()
{
	...
	CMI_LSupp LSupp;
	/////////////////////
	// m_pCurrentSupp saved in ShowInfo_...
	SUPPCARD supCard	= *(m_pCurrentSupp->GetSuppCard());
	/////////////////////
	m_pCurrentSupp = LSupp.CreateNodeBased(SuppProc,supCard,m_pCurrentNode,NODE);
	////////////////////////////////////////////////////////////////////// Show
...
int CSupMdit::ShowCurveLinSuppToEdit()
{
	...
	SUPPCARD supCard	= *(m_pCurrentSupp->GetSuppCard());
	/////////////////////
//	m_pCurrentSupp = LSupp.CreateCurveBased(SuppProc,supCard,m_pCurrentCurve,CURVE);
	////////////////////////////////////////////////////////////////////// Show
	int nActiveIndex = pObjectMgr->GetActiveObjectIndex();	// Get Active Index									
	pObjectMgr->SetActiveObjectIndex(nActiveIndex);		    // set Active Index									
	pObjectMgr->SetActiveObjectType(LIN_SUPPORT);		    // set Active Type									
...
int CSupMdit::ShowPatchLinSuppToEdit()
{
...
//		m_pCurrentSupp = LSupp.CreatePatchBased_4(SuppProc,supCard,m_pCurrentPatch,PATCH);
//	if(m_pCurrentPatch->GetPatSubType() == PS_TRIANGLE)
//		m_pCurrentSupp = LSupp.CreatePatchBased_3(SuppProc,supCard,m_pCurrentPatch,PATCH);
	////////////////////////////////////////////////////////////////////// Show
	int nActiveIndex = pObjectMgr->GetActiveObjectIndex();	// Get Active Index									
	pObjectMgr->SetActiveObjectIndex(nActiveIndex);		    // set Active Index									
	pObjectMgr->SetActiveObjectType(LIN_SUPPORT);		    // set Active Type									
...
int CSupMdit::ShowSolidLinSuppToEdit()
{
	return -1;
}

void CSupMdit::OnEBoxLButtonDown_SUP(CClientDC* pdc,CView* pView,GENERATE gener,
...
int CSupMdit::ShowInfo_NodeLinSupport(CDrFE0D* pDrNode, char* buf, int jStart)
{
	int j = jStart;
	////////////////////////////////////////
	...
	if(pList->GetCount() > 2)
	{
		AfxMessageBox("Internal ERROR:\nCSupMdit::FillInfo_NodeLinSupport\npList->GetCount() > 2"); 
		return -2;
	}
	////////////////////////////////////////////////////////////////////////
	int i;
...
int CSupMdit::FillInfo_Rigid(SUPPCARD supCard,char* buf, int jStart)
{
...
int CSupMdit::FillInfo_SprOrSnu(CString& Nid,SUPPCARD supCard,char* buf, int jStart,BOOL bPat)
{
	CString dir[] = {"X","Y","Z"};
	//////////////////////////////
...
int CSupMdit::LeftUp_NodeLinSupp(CPoint point)
{
	///////////////////////////////////////////////////////////////////////
	CDrGrafDoc* pDoc 			= ((CDrGrafApp*)AfxGetApp())->GetDocument();
	CObjectMgr* pObjectMgr  = pDoc->GetObjectMgr(); 
...
int CSupMdit::ShowInfo_CurveLinSupport(CDrMesh1D* pDrCurve, char* buf, int jStart)
{
	int j = jStart;
	///////////////
	CDListMgr* pList;
	...
	if(pList->GetCount() > 2)
	{
		AfxMessageBox("Internal ERROR:\nCSupMdit::ShowInfo_CurveLinSupport\npList->GetCount() > 2"); 
		return -2;
	}
	////////////////////////////////////////////////////////////////////////
	int i;
...
int CSupMdit::LeftUp_CurveLinSupp(CPoint point)
{
	///////////////////////////////////////////////////////////////////////
	CDrGrafDoc* pDoc 		= ((CDrGrafApp*)AfxGetApp())->GetDocument();
	CObjectMgr* pObjectMgr  = pDoc->GetObjectMgr();
...
*/
			...
			{
				////
				CDrFE0D* pNode = (CDrFE0D*)pList->GetNextObject(posI);
	...
	m_pCurrentCurve			= (CDrMesh1D*)pDListMgr->GetObject(index);
	/////////////////////////////
	nResult = ShowCurveLinSuppToEdit();
	///////////
	if( nResult == -1)		// 
	{
		AfxMessageBox("ERROR:\nCSupMdit::LeftUp_CurveSup\nShowLinSup Problem");
		return -1;
	}
	else
	if(nResult == -2)		// Cancel
...
int CSupMdit::ShowInfo_PatchLinSupport(CDrMesh2D* pDrPatch, char* buf, int jStart)
{
	int j = jStart;
	///////////////
	CDListMgr* pList;
	...
	if(pList->GetCount() > 2)
	{
		AfxMessageBox("Internal ERROR:\nCSupMdit::ShowInfo_PatchLinSupport\npList->GetCount() > 2"); 
		return -2;
	}
	////////////////////////////////////////////////////////////////////////
	int i;
...
int CSupMdit::LeftUp_PatchLinSupp(CPoint point)
{
	///////////////////////////////////////////////////////////////////////
	CDrGrafDoc* pDoc 		= ((CDrGrafApp*)AfxGetApp())->GetDocument();
	CObjectMgr* pObjectMgr  = pDoc->GetObjectMgr();
...
*/
	...
	m_pCurrentPatch			= (CDrMesh2D*)pDListMgr->GetObject(index);
	/////////////////////////////
	nResult = ShowPatchLinSuppToEdit();
	///////////
	if( nResult == -1)		// 
	{
		AfxMessageBox("ERROR:\nCSupMdit::LeftUp_PatchSup\nShowLinSup Problem");
		return -1;
	}
	else
	if(nResult == -2)		// Cancel
...
int CSupMdit::ShowInfo_SolidLinSupport(CDrMesh3D* pDrSolid, char* buf, int jStart)
{
	return -1;
}

int CSupMdit::LeftUp_SolidLinSupp(CPoint point)
{
	return -1;
}

void CSupMdit::ElementLeftUp_SUP(CPoint point)
{                          
...
void CSupMdit::OnEBoxMouseMove_SUP(CClientDC* pdc,CView* pView,GENERATE gener,
											UINT nFlags, CPoint point)
{  
...
void CSupMdit::OnEBoxLButtonUp_SUP(CClientDC* pdc,CView* pView,GENERATE gener,
											UINT nFlags, CPoint point)
{
	m_nFlags				= nFlags;
	m_pView					= pView;
...
void CSupMdit::OnEBoxRButtonDown_SUP(CClientDC* pdc,CView* pView,GENERATE gener,
									UINT nFlags, CPoint point)
{ 
	m_nFlags				= nFlags;
	m_pView					= pView;
...
void CSupMdit::OnEBoxRButtonUp_SUP(CClientDC* pdc,CView* pView,GENERATE gener,
								UINT nFlags, CPoint point)
{
	m_nFlags				= nFlags;
	m_pView					= pView;
...
////////////////////////////////////////////////////////////////////	
UINT CSupMdit::ReSetToolID_SUP(UINT CntlID,UINT IGenID, UINT OperID)
{
	CDrGrafFrame* pWnd	= (CDrGrafFrame*)AfxGetMainWnd();
	CString strCur;
	////////////////
...
void CSupMdit::Serialize(CArchive& ar)
{

	CSolMdit::Serialize( ar);              // must call base class Serializer
	////////////////////////////
	if (ar.IsStoring())
	{
		TRACE(" SupMdit:    Storing\n");	
