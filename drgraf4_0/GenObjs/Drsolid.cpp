...
{  
    ...
    //////////////////////////////  
	m_SolidProc			= SP_COONSHEXA;
	m_SolidType			= SO_COONS;
...
//	m_dAng_2_R			= ;
	/////////////////////////////////////////// Duct/Sweep
	m_bIncTwist_R		= FALSE;	// Incremental?
	m_bIncScale_R		= FALSE;	// for Sweep
/*
	m_TwistMult_R		= ;
	m_ScaleMult_R		= ;	// for Sweep
	m_TwistStart_R		= ;
	m_ScaleStart_R		= ;	// for Sweep
*/
	//////////////////////////////////////////
	m_OutMethod_R		= SO_HORNER;		// Output Method
	m_bUniform_R		= TRUE;		// Output Distribution:
										// 	true 	= Uniform
										//	false 	= Non-Uniform
	m_nMaxOutSeg_R		= -1;		// Number of Output Segments on ENTIRE Curve in T
	m_pNum_t_R			= NULL;			// Number of Outputs for each Output Segment
											//	Num_t[i], i = 1,nMaxOutSeg																							
	m_pDis_t_R			= NULL;			// Number of Outputs for each Linear Single Segment
											//	Dis_t[i], i = 1,nMaxOutSeg																							
	m_pDis_Ang_R		= NULL;		// Output theta-distribution array for Circle:
											//	Dis_Ang[i], i = 1,m_nSegUnif_T+1
	m_pNum_t_Ex_R		= NULL;		// Expanded Number of Outputs for each Output Segment
	m_pNum_t_ST			= NULL;			// Number of Outputs for each Output Segment
											//	Num_t[i], i = 1,nMaxOutSeg																							
	m_pDis_t_ST			= NULL;			// Number of Outputs for each Linear Single Segment
											//	Dis_t[i], i = 1,nMaxOutSeg																							
	m_pDis_Ang_ST		= NULL;		// Output theta-distribution array for Circle:
											//	Dis_Ang[i], i = 1,m_nSegUnif_T+1
	m_pNum_t_Ex_ST		= NULL;		// Expanded Number of Outputs for each Output Segment
	//////////////////////////////////////////////////////////////////////////////// Interpol																																		
	m_nOrder_R			= 2;			// k=2: Linear 
	m_nDim_R			= 3;			// 4 = RATIONAL/	3 = NONrational 
	//////////////////////////////////////////////////////////////////// CONICS
	m_nData_Ex_R		= -1;	// GIVEN: # of Data 		= m_nMaxINodes_S  
	m_bExpanded_R		= FALSE;	
	m_nMaxENodes_R		= -1;
/*
	m_Center_R			= ;	// Circle
...
*/
	...
	m_bHole_R			= FALSE;
	//////////////////////////////////////////////////////////////////// Derivatives
	m_pTangents_R		= NULL;			// Tangents
	m_pCurvatures_R		= NULL;		// Curvatures
    //////////////////////////////////////////////////////////////////// Output
    m_Ratio_R			= 1.0;				// for OneSegment Linear Only
	//////////////////////////////////////////////////////////////////// Triangular Patch
	m_VPatchList.RemoveAll();       //
	/////////////////////////
	m_TetraList.RemoveAll(); 
	m_PentaList.RemoveAll(); 
	m_HexaList.RemoveAll(); 
	m_nTriangles_ST		= -1;			//  Total No of Triangles in Base Patch
	//////////////////////////////
} 
...
//		m_PatchList.AddTail(&(pAdd->m_PatchList) ); 
   ///////////////////////////
	...
	/////////////////////
	m_pFromIDNode_R			= pAdd->m_pFromIDNode_R;
	m_pToIDNode_R			= pAdd->m_pToIDNode_R;
//	m_CurveID			= pAdd->m_CurveID;
	/////////////////////////////////////////// Rotate
	...
	//////////////////////////////////////////////////////////////////// Triangular Patch
	if(!(pAdd->m_VPatchList.IsEmpty()) )
		m_VPatchList.AddTail(&(pAdd->m_VPatchList) );
	////////////////////////////////////////////////////
	if(!(pAdd->m_TetraList.IsEmpty()) )
		m_TetraList.AddTail(&(pAdd->m_TetraList) ); 
	if(!(pAdd->m_PentaList.IsEmpty()) )
		m_PentaList.AddTail(&(pAdd->m_PentaList) ); 
	if(!(pAdd->m_HexaList.IsEmpty()) )
		m_HexaList.AddTail(&(pAdd->m_HexaList) ); 
...
//		pAdd->m_PatchList.AddTail(&m_PatchList ); 
   ///////////////////////////
	...
	/////////////////////
	pAdd->m_pFromIDNode_R		= m_pFromIDNode_R;
	pAdd->m_pToIDNode_R			= m_pToIDNode_R;
//	pAdd->m_CurveID			= m_CurveID;
	...
	//////////////////////////////////////////////////////////////////// Triangular Patch
	if(!(m_VPatchList.IsEmpty()) )
		pAdd->m_VPatchList.AddTail(&m_VPatchList );
	///////////////////////////////////////////////
	if(!(m_TetraList.IsEmpty()) )
		pAdd->m_TetraList.AddTail(&m_TetraList ); 
	if(!(m_PentaList.IsEmpty()) )
		pAdd->m_PentaList.AddTail(&m_PentaList ); 
	if(!(m_PentaList.IsEmpty()) )
		pAdd->m_HexaList.AddTail(&m_HexaList ); 
...
void CDrSolid::Pack_drPipe(CString* pstrCard)
{
	...
	static CString name;
   	/////////////////////////
   	for (int i = 0;i<nMax_SolidTypes;i++)
   		if(SoTypes[i].num == SOLID) 
   		{
...
//////////////////////////////////////////////////// View Matrices
	...
	if(1 != 1 ) // 
	{
 	   ...
 	   ///////////////////////////////////////////
		sprintf(buf,"CDrSolid::Generate_SolidHex-Just before Calling TensorProduct_3D");
		AfxMessageBox(*str);
		///////////////////
		int i=-1,j,k,l,m;
	    ///////////////////
		j = sprintf(buf,"Control Nodes:m_pIn[%d,%d,%d] in Eye Coords",
							m_nMaxCNodes_S,m_nMaxCNodes_T,m_nMaxCNodes_R);
		for(m=0;m<m_nMaxCNodes_R;m++)
		{
			for(k=0;k<m_nMaxCNodes_T;k++)
			{
				for(l=0;l<m_nMaxCNodes_S;l++)
				{
					i++;
		   			j += sprintf( buf + j, "RTS(%d,%d,%d)   %7.2f %7.2f %7.2f \n", m, k, l, 
					(m_pIn+i)->x, (m_pIn+i)->y, (m_pIn+i)->z );
				}
	  			j += sprintf( buf + j, "\n");
			} 
	  		j += sprintf( buf + j, "\n");
...
{ 
	...
	for(i=0;i<=nOutSeg_S;i++)
	{
		...
		for(j=0;j<=(nOutSeg_S-i);j++)
		{
			...
			jOut	= jOutGlo_ST + j;
			////
			for(nTr=1;nTr<nTrMax;nTr++)
			{
				//////////////////////////////////// Barycentric to Linear: Global
				L1 = BARY2LIN_PT(iOut,jOut,m_nOutGlo_S) + (nTr-1) * nOutGlo_ST + nOutStartIndex;
				//////////////////////////////////// Barycentric to Linear: Global
				L2 = BARY2LIN_PT(iOut,jOut,m_nOutGlo_S) + nTr * nOutGlo_ST + nOutStartIndex;
				////
				DrawLine3D(PtrDC,m_pOut+L1,m_pOut+L2,TRUE);// TRUE = XFormed; FALSE = WORLD 
...
BOOL CDrSolid::IsObjectHit(int nNet,POINT point)
{
	
	if(m_SolSubType == SS_TETRA)
		return IsObjectHit_TETRA(nNet,point);
	else
	if(m_SolSubType == SS_PENTA)
		return IsObjectHit_PENTA(nNet,point);
	else
	if(m_SolSubType == SS_HEXA)
		return IsObjectHit_HEXA(nNet,point);
	///////////////////////////////
	else
		return FALSE;
}

BOOL CDrSolid::IsObjectHit_TETRA(int nNet,POINT point)
{
...
BOOL CDrSolid::IsObjectHit_PENTA(int nNet,POINT point)
{
	if(ObjectHit_Generate2DPts()<0)
		return FALSE;
	///////////////////////////////////////////////// Generated Nodes in 2DScreen Coordinates
...
//		int	nOutStart	= pPen->GetOutStartIndex();			// Overall Degree of Triangle
		...
		if(nType == 1)
		{
			if(ObjectHit_PENTA_1(nTrMax,nOutSeg_S,iOutGlo_ST,jOutGlo_ST,nOutStartIndex,
									nNet,point) )
			{
				//////////////////////////////////// Free Memory
				if(m_pOutDP)
		...
		if(nType == 2)
		{
			if(ObjectHit_PENTA_2(nTrMax,nOutSeg_S,iOutGlo_ST,jOutGlo_ST,nOutStartIndex,
									nNet,point) )
			{
				//////////////////////////////////// Free Memory
				if(m_pOutDP)
		...
		{
			//////////////////
			AfxMessageBox("CDrSolid::ObjectHit_PENTA\nnType !==1 or 2");
			return FALSE;
		}
	}										///////////////////////////// end LIST
	//////////////////////////////////// Free Memory
...
BOOL CDrSolid::ObjectHit_PENTA_1(int nTrMax,int nOutSeg_S,int iOutGlo_ST,int jOutGlo_ST,
...
BOOL CDrSolid::ObjectHit_PENTA_2(int nTrMax,int nOutSeg_S,int iOutGlo_ST,int jOutGlo_ST,
									int nOutStartIndex,int nNet,POINT point)
{ 
	//////////////////////	
	int	nMinX,nMaxX;
...
BOOL CDrSolid::IsObjectHit_HEXA(int nNet,POINT point)
{
	if(ObjectHit_Generate2DPts()<0)
		return FALSE;
	//////////////////////	
...
int CDrSolid::ObjectHit_Generate2DPts()
{
