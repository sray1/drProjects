...
CDlg_1Pro::CDlg_1Pro(CWnd* pParent /*=NULL*/)
	: CDialog(CDlg_1Pro::IDD, pParent)
{
	buttonOK.LoadBitmaps("Image1Up","Image1Down","Image1Focus");
	buttonCancel.LoadBitmaps("Image2Up","Image2Down","Image2Focus");
	//{{AFX_DATA_INIT(CDlg_1Pro)
	m_CPro_ID 	= "";
	m_Orient = _T("");
	//}}AFX_DATA_INIT
	m_End_NID	= "";
...
void CDlg_1Pro::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDlg_1Pro)
	DDX_Text(pDX, IDC_CID_PROF, m_CPro_ID);
	DDX_CBString(pDX, IDC_ORIENT, m_Orient);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CDlg_1Pro, CDialog)
	//{{AFX_MSG_MAP(CDlg_1Pro)
...
/////////////////////////////////////////////////////////////////////////////
// CDlg_1Pro message handlers

BOOL CDlg_1Pro::OnInitDialog()
{
	...
	int nMaxCProfs 	 = pDListMgr->GetSpecificObjectCount(M1_PROFILE);
	if(nMaxCProfs)
...
void CDlg_1Pro::OnDefaultLmat() 
{
	m_bLMat		= !m_bLMat;
}

void CDlg_1Pro::OnDel()
{
...
void CDlg_1Pro::OnAdd()
{
	...
	if(index>=0)
	{
		...
		{
			int nAnswer = 
			AfxMessageBox("Do You Want to Select this Mesh1D Again?",MB_YESNO|MB_ICONQUESTION);
			if(nAnswer == IDNO)
				return; //
	...
	////////////////////////////////////////////////////////////
	if((index= pObjectMgr->GetObjectIndexfromKey(MESH1D,CNid,1)) <0)
	{
		AfxMessageBox("Internal Problem:CDlg_1Pro::OnAdd");
		return;
	}
	////////////
	CDrFE0D* pNode;
	pDListMgr	= pObjectMgr->GetObjectList(MESH1D);
	CDrMesh1D* pCurve = (CDrMesh1D*)pDListMgr->GetObject(index);
	CDListMgr* pList = pCurve->GetCNodeList();
	if(!pList->IsEmpty())
	{
		if(m_CuLat == CL_FORWARD)
			pNode = (CDrFE0D*)pList->GetHead();
		else
			pNode = (CDrFE0D*)pList->GetTail();
	}
	else
	{
		AfxMessageBox("Internal Problem:CDlg_1Pro::OnAdd\nEmptyList");
...
//			Math3D.F_MultMATRIX(&m_LocalM,pM,&m_LocalM);
	...
	////////////
	pCurve = (CDrMesh1D*)pDListMgr->GetObject(index);
	pList = pCurve->GetFE0DList();
	if(!pList->IsEmpty())
	{
		if(m_CuLat == CL_FORWARD)
			pNode = (CDrFE0D*)pList->GetTail();
...
void CDlg_1Pro::SetUpLocalm(pMATRIX pM)
{
	///////////////////////////////////////////////////// 			 
	C3DMath	Math3D;
	CXForm	XForm;
...
void CDlg_1Pro::GetTranslations() 
{
	CXForm	XForm(TRUE);
	////////////////////
	CString Nid;
...
void CDlg_1Pro::EmptyListAllString(UINT DlgID)
{
...
void CDlg_1Pro::OnOK()
{
	...
	/////////////////////////////////////////////////////////////////  ADD
	int nActiveIndex;
    CDrProf_1* pAdd 	= (CDrProf_1*)(pObjectMgr->AddToDataBase(nActiveIndex,M1_PROFILE));
...
void CDlg_1Pro::OnCancel()
{
	////////////////////
	CDialog::OnCancel();
}

void CDlg_1Pro::FillListAllString_Pool()
{
	// with MESH1DS
	///////////////////////////////////////
	CDrGrafDoc* pDoc 		= ((CDrGrafApp*)AfxGetApp())->GetDocument();
...
void CDlg_1Pro::FillListAllString_Profile()
{
	...
	//////////////////////////////////////////////////////
	int nMaxCurves = pDListMgr->GetSpecificObjectCount(M1_PROFILE);
    if(nMaxCurves<=0)
    	return;
	//////////////////////////////////////////////////////
	...
	for (;pos !=NULL;)
	{
		...
		{ 
			CDrProf_1* pCurve = (CDrProf_1*)pObject;
...
void CDlg_1Pro::SetInfoForDataBase(CDrProf_1* pDrObject)
{
	...
	CDrMesh1D* pDrCurve;
	////////////////////////////////////
    pDrObject->SetObjectType((int)M1_PROFILE);
	...
	for (i=0;i<nData;i++)
	{
		...
		//////////////////////////////////
		index = pObjectMgr->GetObjectIndexfromKey(MESH1D,Nid,1);
		pDListMgr	= pObjectMgr->GetObjectList(MESH1D);
		if(index>=0)
		{
