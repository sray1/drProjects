...
////////////////////////////////////
IMPLEMENT_SERIAL(CMS_Swee3,CMI_GangP,1)
/////////////////////////////////////////////////////////////////////////////
CMS_Swee3::CMS_Swee3()
{
}
...
//	SetPatchDataFromCurve(pCurve,2/*nDir*/);
	...
	SolCNCoords_N_WtsAtJns(m_FromPt,
					...
					pTwist,pScale,pSolConsJn,pSolWtsJn,nTotConsJn_STR);
	/////////////////////////////////////////////////////////////////////////////// PostSolidData
	...
	nResult = PostSolBezInfoUsingBlendingInfo(pDrSolid,pCurve/*SpineCurve*/,
					pSolConsJn,pSolWtsJn,m_nMaxSolidCNodes_S,m_nMaxSolidCNodes_ST,nConBZJn_R/*nCurves*/,
					m_nMaxSolidCNodes_R);
	/////////////////// release memory
	delete [] pPatConBZ_ST;
	delete [] pPatWtsBZ_ST;
	...
	nResult = SetInfoForDataBase(pDrSolid,
					...
					pPLatchList_U,pCLatchList_W,pTwistList,pScaleList);
	/////////////////////////////////////////////////////////////////////////////// Done
	pPatchList_U->RemoveAll();
	pCurveList_W->RemoveAll();
...
//////////////////////////////////////////////////////////////////////////////////// PreProcess
BOOL CMS_Swee3::IsDataCompatible(CDListMgr* pNodeList,
...
int CMS_Swee3::TwistProcess(pDOUBLE pTwist, int nTwist, double dScaleMax, double dTwistStart,
																		BOOL bIncremental)
{
	int		i;
	////////////////////////////
...
int CMS_Swee3::ScaleProcess(pDOUBLE pTwist, int nTwist, double dScaleMax, double dTwistStart,
																		BOOL bIncremental)
{
	int		i;
	////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////////// PreProcess
int CMS_Swee3::SetSolidDataFromPatch(CDrPatch* pPatch)
{
	///////////////////////////////////////////////////////////////////////////////////
...
//	m_pNum_t_Ex_T			= pPatch->GetNum_tPointer_Ex_T();
	...
	////////////////////
    m_pTriList				= pPatch->GetTriangleList();
	/////////////////	
...
//	m_nSegs_TCon			= m_nOut_TCon - 1;
	m_nMaxSolidCNodes_S 	= pPatch->GetMaxCNodes_S(); // On Each side of the Triangle
	m_nMaxSolidCNodes_T 	= pPatch->GetMaxCNodes_T(); // TOTAL of the Triangle
	m_nMaxSolidCNodes_ST 	= m_nMaxSolidCNodes_T;
	//////////////////////////////
	m_PatchType				= pPatch->GetPatchType();
...
int CMS_Swee3::GetPatCNodeCoords_N_Wts_XSection(CDrPatch* pPatch, PATCHLATCH pLatch, 
...
//////////////////////////////////////////////////////////////////////////////////////
int CMS_Swee3::GetSolidInfoFromSpine_R(CDrCurve* pCurve)
{
	int i;
	pINT	pNum_t_R;
	pDOUBLE	pDis_t_R;
...
int CMS_Swee3::GetCNodeCoords_N_WtsAtJnPts_Spine(CDrCurve* pCurve, CURVELATCH cLatch, 
									pWORLD pConOld, pDOUBLE pWtsOld)
{
...
int CMS_Swee3::CreateTangentsAtJnPts_Spine(CDrCurve* pCurve,CURVELATCH cLatch,pWORLD pTangents)
{
...
int CMS_Swee3::SolCNCoords_N_WtsAtJns(WORLD wAnchorPt,pWORLD pPatConBZ_ST,pDOUBLE pPatWtsBZ_ST,
							int nPatConBZ_ST,pWORLD pConJnBZ_R,pWORLD pTanJnBZ_R,int nConJnBZ_R,
							pDOUBLE pTwist,pDOUBLE pScale,pWORLD pSolCons,pDOUBLE pSolWts,int nTotOut)
{
	////////////////////////////////////////////// generate Patch Pts
	CPA_Sweep	PA_Sweep;		 	   	
...
int CMS_Swee3::PostSolBezInfoUsingBlendingInfo(CDrSolid* pSolid,CDrCurve* pSpineCurve,
...
//	nCon_T					= (bClosed_T)?(nDegree_T*nBezSeg_T):(nDegree_T*nBezSeg_T+1);
	pDOUBLE	pTemSolWts_BZ	= new double[nCon_ST*nCon_R];
	CString SolID			= pSolid->GetObjectID();
	//////////////////////////////////////////////////////// Loop Over Bez. Pts. on Patches
	for(i=0;i<nCon_S;i++)
	{
		for(j=0;j<nCon_S-i;j++)
		{
			//////////////////////////////////// Barycentric to Linear
			m = BARY2LIN_PT(i,j,nCon_S);
			//////////////////////////////////////////////////// Input Nodes 
	...
	{
		PostPatBezInfo(pSolid,TemCNodeList,pTemSolWts_BZ,nCon_S,nCon_ST,nCon_R);
...
int CMS_Swee3::PostPatBezInfo(CDrSolid* pSolid,CDListMgr& TemCNodeList,pDOUBLE pTemSolWts_BZ,
															int nCon_S,int nCon_ST,int nCon_R)
{
	// All the Comments below is to be seen as only ONE
	//				of nCon_R many Patch Planes
	////////////////////////////////////////////////
	int i,j,k=-1,m,r,index;
	CDListMgr* pCNodeList	= pSolid->GetCNodeList();
	pDOUBLE	pSolWts_BZ		= new double[nCon_ST*nCon_R];
	CDrNode* pNode;
	double Wts;
	POSITION pos;
	/////////////////////
...
int CMS_Swee3::TempPostSolBezJunctionPtsInINodeList(pWORLD pConBZ,int nConBZ_ST,int nRowCol,
...
int CMS_Swee3::PackSolBezJunctionWtsInDataWtsArray(pDOUBLE pWtsBZ,int nConBZ_ST,int nRowCol,
...
int CMS_Swee3::GenerateSolidBezInfoFromSpineCurve(
...
int CMS_Swee3::Copy_N_TransferBezControlInfo(CDrCurve* pCurve,CDListMgr* pSolCNodeList,
...
int CMS_Swee3::Copy_N_TransferBezWeightInfo(CDrCurve* pCurve,pDOUBLE pPatWts_BZ,int nCon_R,int nRowCol)
{
	pDOUBLE pWts	= pCurve->GetpWts_BZ_S(); 
...
/////////////////////////////////////////////////////////////////////////////////////
int CMS_Swee3::SetInfoForDataBase(CDrSolid* pDrObject,
...
)
{
	...
	////////////////////////////////////////////////// 
	pDrObject->NumberOfBezierPenta();
	////////////////////////////////////// Now Store PentaHedron Database
	InsertPentaHedrons(pDrObject);
	////////////////////////////////////////////////////////////////////////////////	
...
void CMS_Swee3::SetInfoForDataBase_1(CDrSolid* pDrObject,
...
//	CScaleFactor* pScaleFactor 	= (CScaleFactor*)pObjectMgr->GetSpecObject(SCALEFACTOR);
	...
	//
	if(m_nScaleType == 1)
		pDrObject->GetScaleList_R()->AddTail(pScaleList);
	///////////////////////////////////////////////// Twist Multiplier
	pDrObject->SetIncTwist_R(m_bIncrement);
	pDrObject->SetTwistMult_R(m_TwistMult);
	pDrObject->SetTwistStart_R(m_TwistStart);
	///////////////////////////////////////////////// Scale Multiplier
	pDrObject->SetIncTwist_R(m_bIncScale);
	pDrObject->SetTwistMult_R(m_ScaleMult);
	pDrObject->SetTwistStart_R(m_ScaleStart);
	///////////////////////////////////////////////////////////////// All Other
...
//	pDrObject->SetUniform_T(m_bUnifOT_T);		// Output Distribution:
	...
	/////////////////////////////////////////////////////////////////////////////////////
	pDrObject->SetOutSegUnif_S(m_nMaxOSeg_S); 	// for Old Compatibility:Triangle
												// Output Segments for each Base Triangle
												// on Each Side
	/////////////////////////////////////////////////////////////////////////////////////
...
//?
	...
	pDrObject->SetnOutGlo_S(m_nOut_S);
	//
	pDrObject->SetOut_R(m_nOut_R);
	//////////////////////////////////////////
	pDrObject->SetPatchType(m_PatchType);
	...
	pDrObject->SetMaxCNodes_T(m_nMaxSolidCNodes_T);	// Total_ST
...
//////////////////////////////////////////////////////////////////////////////////////
void CMS_Swee3::Serialize(CArchive& ar)
{
