...
////////////////////////////////////
IMPLEMENT_SERIAL(CSLDPat,CSLDCur,1)
/////////////////////////////////////////////////////////////////////////////
CSLDPat::CSLDPat()
{
}
...
void CSLDPat::GetObjectInfo(CSLDObj* pObject)
{
	
	CSLDCur::GetObjectInfo(pObject);
	///////////////////////////////////   
	CSLDPat* pAdd = (CSLDPat*) pObject;
	////////////////////////////////////////////////////////////////////// Patch
...
void CSLDPat::SetObjectInfo(CSLDObj* pObject)
{
	
	CSLDCur::SetObjectInfo(pObject);
	///////////////////////////////////   
	CSLDPat* pAdd = (CSLDPat*) pObject;
	////////////////////////////////////////////////////////////////////// Patch
...
int CSLDPat::ShowPatchStaticLoad(CDrPatch* pCurrentPatch,CString* pID)
{
	...
	if(pCurrentPatch->GetPatSubType() == PS_QUADRILAT)
		nResult = Static.CreatePatchBased_4(pCurrentPatch,&TempList);	// TRUE = NonUniform
	if(pCurrentPatch->GetPatSubType() == PS_TRIANGLE)
		nResult = Static.CreatePatchBased_3(pCurrentPatch,&TempList);	// TRUE = NonUniform
	///
	stLCard = TempList.GetTail(); 
	pCurrentPatch->GetStaLCardList()->AddTail(stLCard);
	////////////////////////////////////////////////////////////////////// Show
	pObjectMgr->ShowObject_On_Off(pCurrentPatch,PATCH,TRUE);
/*
	CDListMgr* pStaLList = pCurrentPatch->GetStaLList(); 
...
void CSLDPat::RemoveMostListsFromPatch(CDrPatch* pAdd)
{
...
int CSLDPat::ShowInfo_PatchStaticLoad(CDrPatch* pDrPatch, char* buf, int jStart
																	,CString& CurrentStLID)
{
	...
	for(i=0;i<pList->GetCount();i++)
	{
		...
		{
			AfxMessageBox("Internal ERROR:\nCSLDPat::FillInfo_PatchStaticLoad\npDrStatic=NULL"); 
			return -2;
		}
		/////////////////////////////////////////
...
int CSLDPat::Get_Values_Iso_Pat_Curve(CDrCurve* pCurveIsoTo,int indexCT,int indexCR,int IndexSkipNode,
...
int CSLDPat::Get_Values_NonIso_Pat(STALPROC StaLProc)
{
	AfxMessageBox("Get_Values_NonIso_Pat\nNOT IMPLEMENTED YET");
	return -1;
}
			
int CSLDPat::Get_Values_Iso_Pat(CDrPatch* pPatch,STALPROC StaLProc,CString& CurrentStPID)
{                          
...
int CSLDPat::Get_Values_IsoNod_AnyProc(CString& NodeID,
...
{									
	...
	if(index<0)
	{
		...
		index		= pDListMgr->GetObjectIndex(CNODE, NodeID);
		///////////////////////////////////////////
		if(index<0)
		{
			...
			int j;
			//////////
			j = sprintf(buf,"CSLDPat::Get_Values_IsoNod_AnyProc\n%s\n%s NodeIndex<0!",strProc,NodeID);
			AfxMessageBox(*str);
		  	///////////////////
		 	delete str;
			return -1;
...
int CSLDPat::Get_Values_IsoCur_AnyProc(CString& CurveID,int iT,int iR,int IndexSkipNode,
...
{									
	...
	if(index<0)
	{
		...
		int j;
		//////////
		j = sprintf(buf,"CSLDPat::Get_Values_IsoCur_AnyProc\n%s\n%s CurveIndex<0!",strProc,CurveID);
		AfxMessageBox(*str);
    	///////////////////
    	delete str;
		return -1;
...
int CSLDPat::Get_Values_Iso_EXTRUDE_Type1(CDrPatch* pPatchIsoTo, STALPROC StaLProc
																,CString& CurrentStPID)
{
	...
	m_pSNodeT = NULL; 
	m_pSNodeR = NULL; 
	NodeID = pPatchIsoTo->GetFromIDNode_T()->GetObjectID();
	/////////////////////////////////////
	strCurr = "Ready for Extrusion@";
	...
	m_pSNodeT = NULL; 
	m_pSNodeR = NULL; 
	NodeID = pPatchIsoTo->GetToIDNode_T()->GetObjectID();
	/////////////////////////////////////
	strCurr = "Ready for Extrusion@";
...
int CSLDPat::Get_Values_Iso_EXTRUDE_Type2(CDrPatch* pPatchIsoTo, STALPROC StaLProc,
																CString& CurrentStPID)
{
...
int CSLDPat::Get_Values_Iso_ROTATE(CDrPatch* pPatchIsoTo, STALPROC StaLProc
																,CString& CurrentStPID)
{
	///////////////////////////////
...
int CSLDPat::Get_Values_Iso_LOFT2(CDrPatch* pPatchIsoTo, STALPROC StaLProc
																,CString& CurrentStPID)
{
...
int CSLDPat::Get_Values_Iso_COONS(CDrPatch* pPatchIsoTo, STALPROC StaLProc
																,CString& CurrentStPID)
{
	...
	///////////////////////////////////////////
	int nCur = 3;
	if(pPatchIsoTo->GetPatchProc() == PP_COONSQUAD)
		nCur = 4;
	/////////////
...
int CSLDPat::Get_Values_Iso_DUCT(CDrPatch* pPatchIsoTo, STALPROC StaLProc
																,CString& CurrentStPID)
{
	//////////////////////////////////////////////////
	...
	m_pSNodeT = NULL; 
	m_pSNodeR = NULL; 
	NodeID = pPatchIsoTo->GetFromIDNode_T()->GetObjectID(); //used for AnchorID
	/////////////////////////////////////
	strCurr = "Ready for Ducting@";
	strCurr += "Anchor Node";
...
int CSLDPat::Get_Values_Iso_SWEEP(CDrPatch* pPatchIsoTo, STALPROC StaLProc
																,CString& CurrentStPID)
{
	//////////////////////////////////////////////////
	//		case 0: "Cross Sectional Curve";
...
int CSLDPat::Generate_IsoPatchControls(CDrPatch* pPatchIsoTo,CString* pID,BOOL bTran)
{                          
...
int CSLDPat::SetUp_IsoPatchControls(CDrPatch* pPatchIsoTo,int indexP,CString* pID,BOOL bTran)
{
...
int CSLDPat::Generate_IsoControls_EXTRUDE(CDrPatch* pCurrentSPatch,CString* pID)
{
	CDrGrafDoc* pDoc 		= ((CDrGrafApp*)AfxGetApp())->GetDocument(); 
	CObjectMgr* pObjectMgr  = pDoc->GetObjectMgr(); 
	//////////////////////////////////////////////////////////////// Set up
	...
	//////////
	double		dLen			= pCurrentSPatch->GetLength_T();
	WORLD		wLen			= pCurrentSPatch->GetWorldLength_T();
	double		Ratio			= pCurrentSPatch->GetRatio_T();		
	/////////////////////////////////////////////////////////////////////////// Fill Info/Do it
	CMP_Xtrud	MP_Xtrud;
	int nResult = MP_Xtrud.GoDoIt(2,FALSE,*pID,pCurrentSPatch,&m_NodeList,&m_CurveList,
								&m_LatchList,m_ElSegList,wLen,dLen,Ratio,m_pView);
	//////////////
	if(nResult<0)
		return -1;
	//////////////////////////////////////////////////////////
...
int CSLDPat::Generate_IsoControls_ROTATE(CDrPatch* pCurrentSPatch,CString* pID)
{
	AfxMessageBox("CSLDPat::Generate_IsoControls_ROTATE\nNOT IMPLEMENTED YET");
	return -1;
}

int CSLDPat::Generate_IsoControls_LOFT2(CDrPatch* pCurrentSPatch,CString* pID)
{
...
int CSLDPat::Generate_IsoControls_DUCT(CDrPatch* pCurrentSPatch,CString* pID)
{
	//////////////////////////////////////////////////////////////////////////////
	// Currently(03/05/97):
	...
	//////////
	int			nTwistType		= pCurrentSPatch->GetTwistType_T();
	...
	CList<double,double>*
				pTwistList		= pCurrentSPatch->GetTwistList_T();
	/////////////////////////////////////////////////////////////////////////// Fill Info/Do it
	CMP_Duct	MP_Duct;
	nResult = MP_Duct.GoDoIt(FALSE,nTwistType,*pID,pCurrentSPatch,&m_NodeList,&m_CurveList,
							&m_LatchList,pTwistList,dTwistMult,bIncrement,dTwistStart,m_pView);
	//////////////
	if(nResult<0)
		return -1;
	//////////////////////////////////////////////////////////
...
int CSLDPat::Generate_IsoControls_SWEEP(CDrPatch* pCurrentSPatch,CString* pID)
{
	//////////////////////////////////////////////////////////////////////////////
	// Currently(03/05/97):
	//		WE DONOT ALLOW DIFFERENT TWISTMULT, TWISTSTART, SCALEMULT, SCALESTART
	...
	CList<double,double>*
				pTwistList		= pCurrentSPatch->GetTwistList_T();
	//
	int			nScaleType		= pCurrentSPatch->GetTwistType_T();
	double		dScaleMult		= pCurrentSPatch->GetScaleMult_T();
	BOOL		bIncScale		= pCurrentSPatch->IsIncScale_T();		
	double		dScaleStart		= pCurrentSPatch->GetScaleStart_T();		
	CList<double,double>*
				pScaleList		= pCurrentSPatch->GetScaleList_T();
	/////////////////////////////////////////////////////////////////////////// Fill Info/Do it
	CMP_Sweep	MP_Sweep;
	nResult = MP_Sweep.GoDoIt(FALSE,nTwistType,nScaleType,*pID,pCurrentSPatch,&m_NodeList,&m_CurveList,
							&m_LatchList,
...
int CSLDPat::Generate_IsoControls_COONS(CDrPatch* pCurrentSPatch,CString* pID)
{
	CDrGrafDoc* pDoc 		= ((CDrGrafApp*)AfxGetApp())->GetDocument(); 
	CObjectMgr* pObjectMgr  = pDoc->GetObjectMgr();
	int i;
	...
	if(ppKind == PP_COONSTRI)
	{
		CMP_CooP3	MP_CoonP;
		/////////////////////
		int nResult = MP_CoonP.GoDoIt(FALSE,*pID,pCurrentSPatch,&m_CurveList,
										&m_LatchList);
		//////////////
		if(nResult<0)
			return -1;
...
void CSLDPat::Serialize(CArchive& ar)
{

	CSLDCur::Serialize( ar);              // must call base class Serializer
	////////////////////////////
	if (ar.IsStoring())
	{
