...
/////////////////////////////////////////////////////////////////////////////
CDegElevate::CDegElevate(BOOL bClosed)
{
	m_bClosed = bClosed;
}
				...
				)
{
	...
	for(int nc=0;nc<nSeg_BZ;nc++)
	{
		...
		for(m=0;m<=nDegOld;m++)
		{
			pConOtem[m] = ((nc == (nSeg_BZ-1) && (m == nDegOld)) && m_bClosed)?
								pConOld[0]:pConOld[nBegOld+m];
			pWtsOtem[m] = ((nc == (nSeg_BZ-1) && (m == nDegOld)) && m_bClosed)? 
								pWtsOld[0]:pWtsOld[nBegOld+m];
		}
		/////////////////////////////////////// Go Elevate	
		...
		for(m=1;m<=nDegNew;m++)
		{
			if( (nc == (nSeg_BZ-1)) && (m == nDegNew) && m_bClosed)
				continue;
			//////////
			nConNew++;
			///////////////
	...
	if(1 != 1 ) // iDLL = 12 , Module = 0
	{
		...
		int j,k,nSize;
		//////////
		sprintf(buf,"CDegElevate::Elevate_Curve");
		AfxMessageBox(*str);
		///////////////////
		nSize = nConNew;
		j = sprintf(buf,"pConNew[%d]\n",nSize);
		for(k=0;k<nSize;k++)
		{
		   j += sprintf( buf + j, "%2d %7.2f %7.2f %7.2f\n", k,
		   					pConNew[k].x, pConNew[k].y, pConNew[k].z);
		}
		AfxMessageBox(*str);
    	///////////////////
		j = sprintf(buf,"pWtsNew[%d]\n",nSize);
		for(k=0;k<nSize;k++)
		{
