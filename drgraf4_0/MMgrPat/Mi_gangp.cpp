...
CMI_GangP::CMI_GangP()
{
	m_pCornerTwist_T	= NULL;// it is full,if TW_USER
	m_pODis				= NULL;
	m_pOSeg				= NULL;
	///////////////
	m_pOut				= NULL;
...
//////////////////////////////////// Need this for Curve Extraction fronm Patch
int CMI_GangP::SetPatchOutputDataFromCurve(CDrCurve* pCurve,int nDir)
{
	int i,nSegs,nOut;
	pINT	pNum_t;
	pDOUBLE	pDis_t;
	///////////////
	pNum_t		= pCurve->GetNum_tPointer_S();
	pDis_t		= pCurve->GetDis_tPointer_S();
	nSegs		= pCurve->GetMaxBezSegments_S(); 
	...
	switch (nDir)
	{
		case 1:
			...
			///
			m_Ratio_S		= pCurve->GetRatio_S();
			m_nOut_S		= nOut;
			m_nMaxOSeg_S	= pCurve->GetMaxOutSeg_S();
			m_bUnifOT_S		= pCurve->IsUniform_S();
			/////////////////////////////////////
			if(pNum_t)
				m_pNum_t_S		= new int[nSegs];
			if(pDis_t)
				m_pDis_t_S		= new double[nOut];
			//////////////////////////////////////////////////
			for (i=0;i<nSegs;i++)
			{
				if(pNum_t)
					m_pNum_t_S[i]		= pNum_t[i];
			}
			for (i=0;i<nOut;i++)
			{
				if(pDis_t)
					m_pDis_t_S[i]		= pDis_t[i];
			}
			////////
			return MA_OK;
		...
		case 2:
			...
			m_nOut_T		= nOut;
			m_nMaxOSeg_T	= pCurve->GetMaxOutSeg_S();
			m_bUnifOT_T		= pCurve->IsUniform_S();
			/////////////////////////////////////
			if(pNum_t)
				m_pNum_t_T		= new int[nSegs];
			if(pDis_t)
				m_pDis_t_T		= new double[nOut];
			//////////////////////////////////////////////////
			for (i=0;i<nSegs;i++)
			{
				if(pNum_t)
					m_pNum_t_T[i]		= pNum_t[i];
			}
			for (i=0;i<nOut;i++)
			{
				if(pDis_t)
					m_pDis_t_T[i]		= pDis_t[i];
			}
			////////
			return MA_OK;
...
int CMI_GangP::DeleteCurves_LockStock(CDListMgr* pCurveList)
{
	...
	CDListMgr*	pNodList	= pObjectMgr->GetObjectList(NODE);
	///////////////////
	CDrNode*	pNode;
	CDrCurve*	pCurve;
	POSITION	pos;
	int			i,nCount,nIndex;
	CDListMgr	TemNodeList;
	////////////////////////
	nCount = pCurveList->GetCount();
	//////////////////////
	for(i=0;i<nCount;i++)
	{
		pCurve	= (CDrCurve*)pCurveList->GetObject(i);
		pos		= pCurveList->FindIndex(i);
		///////////////////
		{
			nIndex = pCurList->GetObjectIndex(pCurve);
			if(nIndex>=0)
			{
				//////////////////////////////////////////// save Nodes to delete
				TemNodeList.AddTail(pCurve->GetINodeList());
				////
				pObjectMgr->DeleteFromDataBase(nIndex, CURVE);
			}	
		}
		/////
	}
	...
	//////////////////////////////////////////////////////// delete Nodes
	nCount = TemNodeList.GetCount();
	//////////////////////
	for(i=0;i<nCount;i++)
	{
		pNode	= (CDrNode*)TemNodeList.GetObject(i);
		pos		= TemNodeList.FindIndex(i);
		///////////////////
		{
			nIndex = pNodList->GetObjectIndex(pNode);
			if(nIndex>=0)
				pObjectMgr->DeleteFromDataBase(nIndex, NODE);
		}
		/////
	}
	TemNodeList.RemoveAll();
	////////////
	return MA_OK;
}

int CMI_GangP::ReParametrizeBezWeightsToStdForm(CDrCurve* pCurve)
{
	//                     _    _
	// Following will make W0 = Wn = 1.0 <--- Standard Form
	/////////////////////////////////////
	//	c	= nth root(W0/Wn)
	//	_
	//	Wi	= (c^i * Wi)/W0, i = 0,...,n
	...
	int nWts		= pCurve->GetMaxCNodes_S();
	double rw0		= 1.0/pWts[0];
	/////////
	double c		= pow(pWts[0]/pWts[nWts-1],1.0/(double)nWts);
	/////////
...
//	m_nSegs_S			= pDrCurve->GetSegmentCount_S();	// for Uniform Output
	m_nSegs_S			= pDrCurve->GetOutSegUnif_S();	// for Uniform Output
	//////// 
							...
							BOOL bSave)
{
    ...
    for(i = 0; i<nMaxCNodes; i++)              
    {
		...
		if(bSave)
		{
				...
				/////////////////////////////// Reciprocate 
				int index = (pDrNode->GetPatchList())->GetObjectIndex(m_pDrPatch); // exists already?
				if(index>=0)
					continue;
				/////////////
...
//			pAddNode->SetLevelType((int)LEVEL_KIDDO);          // private
			///////////////////////////////////// Do Not Reciprocate:PRIVATE 
//			int index = (pAddNode->GetPatchList())->GetObjectIndex(m_pDrPatch); // exists already?
//			if(index>=0)
//				continue;
			/////////////
...
//	m_pDrPatch->SetSegmentCount_T(m_nSegs_T); 
	...
	if(m_pNum_t_S)
	{
		...
		m_pDrPatch->SetNum_tPointer_Ex_S(m_pNum_t_S);// Circle Special
	}
	else
	if(m_pNum_t_Ex_S)
	{
		m_pDrPatch->SetNum_tPointer_S(m_pNum_t_Ex_S);			// Number of Outputs for each Output Segment
	...
	if(m_pNum_t_T)
	{
		...
		m_pDrPatch->SetNum_tPointer_Ex_T(m_pNum_t_T);// Circle Special
	}
	else
	if(m_pNum_t_Ex_T)
	{
		m_pDrPatch->SetNum_tPointer_T(m_pNum_t_Ex_T);			// Number of Outputs for each Output Segment
...
int CMI_GangP::InsertTriangle_Linear()
{
	...
	for(i=0;i<=nPm1;i += nDeg)
	{
		...
		{
			////////////////////////////////////////////// Store:Type 1
			nC = StoreType_1_Lin(i, j, nP, pTriList);
			////////
			if(nC<0)
				return -1;
			else
		...
		{
			////////////////////////////////////////////// Store:Type 2
			nC = StoreType_2_Lin(i+nDeg, j, nP, pTriList);
			////////
			if(nC<0)
				return -1;
			else
...
//		Nid	= PatchID.Left(nChLen);
		...
		////////
//092798		pAddNode = PostNode(Nid,LocalPos,TRUE,FALSE); 	//CNodes = TRUE
		pAddNode = PostNode(Nid,LEVEL_GRAPA,LocalPos,TRUE); 	//CNodes = TRUE
		pListTemp->InsertObject(pAddNode);
	}
	return 0;		
}

double CMI_GangP::UnitAxis_N_LenFromWorldLength()
{
	///////////////////////////////////////////// Vector
	m_vAxis.x = m_wLen.x;  
	m_vAxis.y = m_wLen.y;  
...
//	WORLD		ptDest_1,ptDest_2;
	////////////////////////////////////////////////////// FromNode
   	///////////////////////////////////////////// get Raw Pt.
   	m_FromPt = *(m_pFromIDNode->GetLocalPos());
	//////////////////////////////// For Sweep/duct
	if(nType == 1)
		return; 
	////////////////////////////////////////////////////// ToNode
   	///////////////////////////////////////////// get Raw Pt.
  	m_ToPt = *(m_pToIDNode->GetLocalPos());
	///////////////////////////////////////////// Vector
...
int CMI_GangP::FillMeshTangentsInOneDirPatch(CDrPatch* pDrPatch, pWORLD pOut, int nIsoDir )
{
//	pDrPatch->RefillCNLocalPosForPosting();	// filling m_pIn in DrCurve w/ LocalPos
	///////////////////////////////////////
	if(	m_PatSubType == PS_TRIANGLE)
		pDrPatch->Generate_Tangent3(pOut, nIsoDir);
	else
		pDrPatch->Generate_Tangent4(pOut, nIsoDir);
	//////////////////////////////////////////		
	if(!m_bBack)
		return 0;
/*
...
int CMI_GangP::FillMeshTangentsPatch(CDrPatch* pDrPatch, pWORLD pOut_S,  pWORLD pOut_T )
{
//	pDrPatch->RefillCNLocalPosForPosting();	// filling m_pIn in DrCurve w/ LocalPos
	///////////////////////////////////////
	if(	m_PatSubType == PS_TRIANGLE)
		pDrPatch->Generate_Tangent3(pOut_S,1,pOut_T,2);
	else
	{
		pDrPatch->Generate_Tangent4(pOut_S, 1);
		pDrPatch->Generate_Tangent4(pOut_T, 2);
	}
	//////////////////////////////////////////		
	if(!m_bBack)
		return 0;
