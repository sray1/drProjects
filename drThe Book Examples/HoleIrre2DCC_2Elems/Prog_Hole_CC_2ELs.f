...
!
	Real(kind=8)	E,PR,Thk,Alfa,Beta,Rad,Trace
...
!---------------------------------------------------------------- Support Cond
	nSup	= 8
	nStifN  = nStifC - nSup	! size NonSingular: yy = xx - 8( 4 supX + 4 SupY)
!-----------------------------------------------------------------------------
	Nint	= 4		! Gauss Order
!	Nint	= 16		! Gauss Order
!----------------------------------------------------
	OPEN(UNIT=10,
     ...
     &HoleIrre2DCC_2Elems\Output.dat')
...
!-----------------------------------------------------------------
	write(iOut,1010) nStifS,iGeom,iCont,nC,nStifC,nSup,nStifN,Nint,
     &				Neltype
	write(iOut,1020) E,PR,Thk,Rad,Alfa,Beta
!-------------------------------------------------------
!	Array sizes
!
!	nSzInt	= 4 !bytes
...
!-------------------------------------------------
	Nel = 1
	call BZ_Coords_EL1(A(n1a),2,nQc,Alfa,Beta,Rad,iOut)
!-------------------------------------|------|--------
...
!-------------------------------------------------
	Nel = 2
	call BZ_Coords_EL2(A(n1b),2,nQc,Alfa,Beta,Rad,iOut)
!-------------------------------------|-----|--------
...
!
	...
	call Quad_CC_LVX(A(n5),A(n4),nStifS,Beta,Iout)
!
!----------------------------------------------------------------------
...
!
 ...
 1020 Format(/ 5x,"Real Controls"/
     ...
     &	2x,"Thickness                  = ",F10.5/
     &	2x,"Hole Radius                = ",F10.5/
     &	2x,"Alfa Multiplier along X    = ",F10.5/
     &	2x,"Beta Multiplier along Y    = ",F10.5/
     &	)		
	end 
!	  			
