...
!------------------------------------------------------------------- Load
	...
	call Get_Loads(A(n5),A(n5V),nLod,nQc1,nQc2,iCont,iOut)
!------------------------------------------------------------------- Solution Params
...
!------------------------------------------------------------------- Reference Properties at Gauss
	...
	nBMat		= n1b			+ nDim*nQc1*nQc2*nSzDbl	! Qc2(nDim,nQc1,nQc2) element 2
	nTMat		= nBMat 		+ nDim*nDof2*nStifEL*nSzDbl
	nEndW		= nTMat 		+ nDim*nDof *nStifEL*nSzDbl
!	--------------
	call Put_Reference_AllGauss(Qc,T,B,Nel)
!------------------------------------------------------------------- Constitutive Matrix
...
!
	...
	write(IoutEnd,1050) float(4),float(iPlotCount),float(0),float(0)
!	------------------------------------------------------------- MatLab Plot End
	iPlot = 1
	if(iPlot == 1) Then
		iPltType = 2
