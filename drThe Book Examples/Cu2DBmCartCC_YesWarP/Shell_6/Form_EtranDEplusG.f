...
!	--------------------------------------------------------------- Constrained	                                              
	Real(kind=8)	GCb             ,EMatCb      ,FMnCb                                 
	Dimension		GCb(nDof2,nDof2),EMatCb(1,18),FMnCb(18)
	Real(kind=8)	GCk             ,EMatCk      ,FMnCk                                 
	Dimension		GCk(nDof2,nDof2),EMatCk(1,18),FMnCk(18)
!	--------------------------------------------------
	Equivalence (rE(1),rE1(1)),(rE(7),rE2(1))
	Equivalence (FMtt,FMt1(1)),(FMtt(7),FMt2(1))
	Equivalence (FMnCb,EMatCb(1,1)),(FMnCk,EMatCk(1,1))
!
...
!
	if (bPen) then
		EMatC = zero
		call FormEMatrix_1_Con(EMatCb,EMatCk,nCMat,nDof2,nDim,1,Iout)
	endif
!	-------------------------------
	CK1		= CK
	AA1		= AA
...
!
	if (bPen) then
		call FormEMatrix_2_Con(EMatCb,EMatCk,nCMat,nDof2,nDim,2,Iout)
	endif
!	-------------------------------
	CK2		= CK
	AA2		= AA
...
!
	...
	GCk = zero
	if (bPen) then
		call FormGMatrix_Con(GCb,nDof2,nDim,nDof,Iout)
	endif
!	--------------------------------------------------------------------- ETransDE+G (18x18)
	EDG	= MATMUL(MATMUL(TRANSPOSE(EMat),DMat),EMat) + G
!======================================================================================= EC and GC: Constraint
!			call DumpRealMatrix(EMatC,nCMat,nDof2,"EMatC",1,iOut)
	penb			= .9950D+2		
	penk = 0.D0
!	penk			= .9950D+1		
	if(bPen) then
		EDG = EDG 
     &		+ penb*(MATMUL(TRANSPOSE(EMatCb),EMatCb) 
     &		+ penk*(MATMUL(TRANSPOSE(EMatCk),EMatCk) 
     &		+ (rE(2)-rE(7))*GCb)+ (rE(5)-rE(10))*GCk)
	endif
!		-------------------------------------------------------- Debug 
	iPrt = 0
	if(iprt == 1) then
...
!
	if (bPen) then
		EFO			= penb*(rE(2)-rE(7))*FMnCb	!EFO(18x1) = pen*ECTrans(18x12)*(beta12-beta21)
