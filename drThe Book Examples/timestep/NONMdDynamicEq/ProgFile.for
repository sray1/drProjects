...
!------------------------------------------------------------------- Heading
	write(iOut,*)
     ...
     &	'==============='
	write(iOut,*)
     &	'   NonLinear MD Dynamic Equation Time Stepping           ',
...
!
     ...
     &				a2*Mass(ii,jj)*ddot(jj)-Stif(ii,jj)*d(jj)
			...
			eLoad(ii) = sum+.5*(Load(ii,is+1)+Load(ii,is)) 
		end do
!
...
!
 ...
 1040 format(//" Load Vectors.........."//11(2x,f8.2) )
