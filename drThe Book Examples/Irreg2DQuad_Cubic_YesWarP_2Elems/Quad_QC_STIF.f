	Subroutine Quad_QC_STIF(Qc,Nel,NelType,Nint,E,PR,Thk,Stif,nStif,
     &	Iout,Trace)
!
...
!		 	
!	Outputs 
!		Stif(xx,xx)	= Desired Stiffness Matrix
...
!
	Real(kind=8)	Stif			 ,Trace,DetNorm
...
!-------------------------------------------------- 
	...
	DATA Rk/	0.5D0,				 0.0D0,						! 1-point		
     ...
     &			0.D0,				 0.0D0,
     &			0.5D0,               0.230765344947158D0,		! 5-point
...
!
!
	write(Iout,1000) Nel,(i,(Qc(i,j),j=1,12),i=1,2)
!
...
!
	Call QC_StrainDispMat(Qc,Ri,Si,B,Det,Nel,NelType,Iout)
!
!	Update determinant Norm
!
	DetNorm = DetNorm + det
!
...
!	done
!----------------------------------------------
	DetNorm = DetNorm/Nint/Nint
	write(Iout,1020) Nel,DetNorm
!----------------------------------------------
	do 110 i = 1,nStif
!	write(iOut,1010) i,(Stif(i,j),j = 1,nStif)
!	Pause
  110	continue
	return
!
 1000 format("QC(2,12): Elem# =",I4/
     &        (I5,5X,4(f10.6,1X)/2(10X,4(f10.6,1X)/)) )
 1010 format("Full Stiffness Matrix:Stif(nStif,nStif)"/
     &        I5/(3(8(f10.6,1X)/)/))
 1020 format(2X,"Nel = ",I5,2X,"detNorm = ",f20.16)
	end   		