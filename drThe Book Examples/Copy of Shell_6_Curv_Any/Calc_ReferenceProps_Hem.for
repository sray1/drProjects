	Subroutine Calc_ReferenceProps_Hem(Qc,tmin,tmax,pmin,pmax,
     &											ri,rj,jCInt)
!	===================================================================
...
!	----------------------------------------------------------- Coords along Lines of Curvatures
	cT	= one			! needed in A1hat routine etc
	sT	= zero
!	----------------------------------------------------------- metric Tensor
...
!
	...
	RutAhatAlt = rad*rad*dcos(prj)*the*phe
!	-------------------------------------------------- Bases: Ao1 & Ao2
...
!	==================================================== Curvature End
	call DotProduct(A1,X11hat,xLam11hat,3,Iout)	! Lamda1,1hat w.r.t zi
...
!
	xLam21 = xLam21hat/xLam1						! lamda2,1 w.r.t s1
	xLam12 = xLam12hat/xLam2						! lamda1,2 w.r.t s2
!	-------------------------------------------------- Curvatures: ko1 & ko2 
	C01(1) = zero
	C01(2) = Cu1
	C01(3) = xLam12/xLam1
!
	...
	C02(2) = zero
	C02(3) = -xLam21/xLam2
!	-------------------------------------------------- Curvatures: ko1 & ko2  CORRECT ONE
	C01(1) = zero
	C01(2) = -Cu1
	C01(3) = -xLam12/xLam1
!
...
!	----------------------------------------------------------- Lines of curvatures
	...
	if(iPrt == 1) then
		...
		call DumpRealVar(RutAhatAlt,"RutAhatAlt",iPrt,iOut)
