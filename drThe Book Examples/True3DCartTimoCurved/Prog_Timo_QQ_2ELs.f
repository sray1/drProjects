...
!
	...
	nV		= nU	+ (nDis+1)*nSzDbl
	nW		= nV	+ (nDis+1)*nSzDbl
	nRx		= nW	+ (nDis+1)*nSzDbl
	nRy		= nRx	+ (nDis+1)*nSzDbl
	nRz		= nRy	+ (nDis+1)*nSzDbl
	nMDR	= nRz	+ (nDis+1)*nSzDbl
	nEnd	= nMDR	+ 6*(nDis+1)*nSzDbl
	nF1		= nEnd
	nF2		= nF1	+ 2*nStr*nSzDbl
	nF3		= nF2	+ 2*nStr*nSzDbl
	nM1		= nF3	+ 2*nStr*nSzDbl
	nM2		= nM1	+ 2*nStr*nSzDbl
	nM3		= nM2	+ 2*nStr*nSzDbl
	nMSA	= nM3	+ 2*nStr*nSzDbl
	nEnd	= nMSA	+ 6*(nStr+1)*nSzDbl
!--------------------------------------------------------------------
!	C0_Condition
!--------------------------------------------------------------------
	if (iCont == 0) then
...
!
	Stop 'OK'
 1000 Format(/ 5x,"TRUE 3D CANTILEVERED: CARTESIAN"/
