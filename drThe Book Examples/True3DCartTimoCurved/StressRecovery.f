...
!
	...
	Dimension		ExMom(nDim),FeStr(6,nDim)
...
!	find Feaxi at s = 0	
	if(Nel == 1) Then
		call Compute_ElementReactions_1(Stif,Qd,nStif,ReI,ReJ,6,
     &    								Qc,nQc,P,Radius,Nel,iOut)
	elseif (Nel == 2) Then 
		call Compute_ElementReactions_2(Stif,Qd,nStif,ReI,ReJ,6,
     &    								Qc,nQc,P,Radius,Nel,iOut)
	endif 
!	------------------------------------------------------- prep. for deriv
	do 20 ii = 1,nQc
