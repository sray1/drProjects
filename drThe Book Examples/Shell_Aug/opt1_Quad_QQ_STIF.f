	Subroutine Opt1_Quad_QQ_STIF(RoCOld,CuCOld,RoSav,CKSav,Qd,
     &						Nel,DMat,B,T,
...
!	================================= Set Engineering or Virtual For/Mom
		bEng = .TRUE.
		bEng = .FALSE.
		call Opt1_FormXforms(Iout)			! Shell to beams coordinate xforms
...
!	====================================== Debug End
	call Opt1_Split_Dis_Rot_Deriv(Qd,nStif,B,Iout)
!	---------------------------------------------------------------- Update Rotation
	do 10 i = 1,nC				
   10	RK(i) = R0(i) + RR(i)
	if(bOpt1) then
...
!	================================================================ ED+G(nDof2,nDof2)
	call Opt1_Form_EtranDEplusGMatrix(DMat,T,B,BVEC,SKLM,EDG,Iout)
	if(bOpt1) then
	endif	
!		-------------------------------------------------------- Debug 
		iPrt = 0
