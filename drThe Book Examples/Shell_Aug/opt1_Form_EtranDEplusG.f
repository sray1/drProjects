	Subroutine Opt1_Form_EtranDEplusGMatrix(DMat,
...
!	Dimension		rE1(nDof),rE2(nDof),FMt1(nDof),FMt2(nDof)                                               
!
...
!	--------------------------------------------------- E(12x18): direction 1 part
	call Opt1_FormEMatrix_1(EMat,nCMat,nDof2,nDim,1,Iout)
!	-------------------------------
	CK1		= CK
	AA1		= AA
	AMat1	= AMat		
...
!	--------------------------------------------------- Real Strains (n-Mn-Qn coords)
	call Opt1_FormRealStrains_Ncoords(rE1,nDof,3,1,Iout)	! strain in shell coords
!======================================================================================= Direction 2
	C0		= C02
!	Hat		= Hat2
	Di		= Di2
...
!	--------------------------------------------------- E(12x18): Add direction 2 part
	call Opt1_FormEMatrix_2(EMat,nCMat,nDof2,nDim,2,Iout)
!	-------------------------------
	CK2		= CK
	AA2		= AA
	AMat2	= AMat		
	WMat2	= WMat		
!	--------------------------------------------------------------------- Real Strains (n-Mn-Qn coords)
	call Opt1_FormRealStrains_Ncoords(rE2,nDof,3,2,Iout)	! strain in shell coords
!======================================================================================= G and E
!--------------------------------------------------------------------- Force-Moments (t-m-b Coords)
!																	and (n-Mn-Qn coords) for G Mat
	if(bOpt1) then
	endif	
	call Opt1_FormForMom(DMat,rE,nCMat,AMat1,AMat2,nC,nDof,FMt1,FMt2,
     &											FMn,FMns,Iout)
!--------------------------------------------------------------------- G(18x18)
	call Opt1_FormGMatrix(G,nDof2,nDim,nDof,Iout)
!--------------------------------------------------------------------- E(12x18)
!	call FormEMatrix(EMat,nCMat,nDof2,nDim,Iout)
!--------------------------------------------------------------------- ETransDE+G (18x18)
