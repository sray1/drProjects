	Subroutine Form_EtranDEplusGMatrix_AUG(Penalt,
     &							AugMul,OldCon,DMat,
...
!
	if (bPen) then
		EMatC = zero
		call FormEMatrix_1_Con(EMatC,nCMat,nDof2,nDim,1,Iout)
	endif
!	-------------------------------
	CK1		= CK
	AA1		= AA
...
!
	if (bPen) then
		call FormEMatrix_2_Con(EMatC,nCMat,nDof2,nDim,1,Iout)
	endif
!	-------------------------------
	CK2		= CK
	AA2		= AA
...
!
	GC = zero
	if (bPen) then
		call FormGMatrix_Con(GC,nDof2,nDim,nDof,Iout)
	endif
!--------------------------------------------------------------------- ETransDE+G (18x18)
...
!			call DumpRealMatrix(EMatC,nCMat,nDof2,"EMatC",1,iOut)
	constrain	= rE(2)-rE(7)
	pen			= Penalt(iGau,jGau)
	if(bPen) then
		EDG = EDG + pen * (MATMUL(TRANSPOSE(EMatC),EMatC) )
!     &		               + pen * (constrain)*GC
	endif
...
!	bAug = .FALSE.
	if(bAug) then
		EDG = EDG + ((AugMul(iGau,jGau) + pen * (constrain)) ) * GC 
	endif
...
!
	if (bPen) then
		EFO			= EFO + pen*(constrain)*FMnC	!EFO(18x1) = pen*ECTrans(18x12)*(beta12-beta21)
		BVEC		= BVEC - MATMUL(TRANSPOSE(BMat),EFO)
	endif
!
	if (bAug) then
		EFO			= EFO + AugMul(iGau,jGau)*FMnC 	!EFO(18x1) = ECTrans(18x12)*(beta12-beta21)
		BVEC		= BVEC - MATMUL(TRANSPOSE(BMat),EFO)
	endif
