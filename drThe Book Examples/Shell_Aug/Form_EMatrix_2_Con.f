!	Subroutine FormEMatrix(AMat,C0,nC,E,nE,W,R,AA,CK,nG,Iout)
	Subroutine FormEMatrix_2_Con(EMat,nE,nG,nC,iDir,Iout)
!	----------------------------------------------------------------------------- 
!	Inputs
...
!	=============================================== 
	call FormAddEBlock_Con(nC,A32,2,Iout)
	E32 = E32 - A32
!	----------------------------------------------------- E12
	E1C = MATMUL(A1H,E31)	
	E2C = MATMUL(A1H,E32)	
...
!----------------------------------------------------------------------- Form EMatrix
	do 40 j = 1,3
	EMat(1 ,j   )	= EMat(1 ,j   ) - E1C(1,j)
	EMat(1 ,j+3 )	= EMat(1 ,j+3 ) - E2C(1,j)
	EMat(1 ,j+12)	= EMat(1 ,j+12) - E3C(1,j)
   40 continue
!---------------------------------------------------------- Write:
	iPrt = 0
	if (iPrt == 1) Then
