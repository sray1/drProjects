	Subroutine Quad_QQ_STIF_AUG(Penalt,AugMul,OldCon,
     &						RoCOld,CuCOld,RoSav,CKSav,Qd,
     &						Nel,DMat,B,T,
...
!		-------------------------------------------------------- Debug : End
!	================================================================ ED+G(nDof2,nDof2)
	call Form_EtranDEplusGMatrix_AUG(Penalt,AugMul,OldCon,
     &								DMat,T,B,BVEC,SKLM,EDG,Iout)
!		-------------------------------------------------------- Debug 
		iPrt = 0
		if((iStep == 9999).AND.(iNut == 0)) Then
!		if((iStep == 0).AND.(iNut == 0)) Then
