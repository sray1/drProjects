...
!		Stif(32,32)	= Desired Stiffness Matrix ( for One Element)
!		AssStif(64,64)	= Assembled Desired Stiffness Matrix ( 2 Elements)
!
...
!
!----------------------------------------------------------------- DATA
	NelTyp	= 0			! Plain Stress
	E		= 1.D0	 	
	PR		= 0.25D0	! Poisson's Ratio
	Thk		= 1.D0		! thickness
	xLx = 4.D0			!Aspect Ratio		
	xLy = 1.D0
!-----------------------------------------------------------------
!
!	Array sizes
!
	...
	n3		= n2 + 32*32*nSzDbl		! S(29,29) Non-Singular
	n4		= n3 + 29*29*nSzDbl		! F(32)
...
! EITHER
!
	call Extract_NonSingularStif(A(n2),32,32,A(n3),29,29)
	call Extract_ContractedLoadVector(A(n4),32,A(n4),29)
!	call Factor_Determinant(A(n3),A(n4),A(n6),A(n7),29,29,29)
	call Solve_AxEqB(A(n3),A(n4),A(n6),29,29)
!
! OR
!	call AddSupport_Stif(A(n2),32,32,Trace)
!	call MultSupport_Load(A(n4),32,Trace)
!	call Solve_AxEqB(A(n2),A(n4),A(n6),32,32)
	call Expand_BezierDispControls(A(n6),29,A(n4),32)
!
...
!	Pause
   80	continue
	return
!
 1010 format("Reduced Stiffness Matrix:Stif(29,29)"/
     &        I5/(3(9(f10.6,1X)/)/))
	return
	end
