...
!
!----------------------------------------------------------------- DATA
!	NelType	= 0			! Plain Stress ( to compare w/ thin beam)
	NelType	= 1			! Plain Stress ( to compare w/ thin beam)
	E		= 1.D0	 	
...
! EITHER
!
	call Extract_NonSingularStif(A(n2),32,32,A(n3),27,27)
	call Extract_ContractedLoadVector(A(n4),32,A(n4),27)
!	call Factor_Determinant(A(n3),A(n4),A(n6),A(n7),29,29,29)
	call Factor_ConditionNumber(A(n3),A(n6),A(n7),27,27,27)
	call Solve_AxEqB(A(n3),A(n4),A(n6),27,27)
	call Compute_StrainEnergy(A(n3),A(n4),A(n6),27,SEnergy,PEnergy)
!
! OR
