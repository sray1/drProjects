...
!----------------------------------------------------
	OPEN(UNIT=10,FILE='c:\The Book Examples\Gauss\detO.dat')
...
!--------------------------------------------------------------------------------------------------
	...
	nQd		= nQc*nDof
!	---------------------------------------------------------------------------------
	nElem	= 2					! no. of Elements/3DLINMOD
!	nElem	= 1					! no. of Elements/BATBUL
!	---------------------------------------------------------------------------------
	nStifEL	=  nDof*nQc 
...
!----------------------------------------------------------------- Solution Params
	...
	call Initiate_SolutionData(Tol,ArcLen,MaxSteps,MaxIt,iCutMax,
     &							iOut)
!     &							iOut,FacLoad)
!-------------------------------------------------------
!	Array sizes
!
	...
	n2d		= n2b    + nStifS*nStifS*nSzDbl	! S(32,32) Singular / 1ELs(n2b)
	n3a		= n2d    + nStifEL*nStifEL*nSzDbl	! T(56,54) Constraint(n3a)
	...
	ndelUiEL	= nEndw			+ 2*(MaxSteps+1)*nSzDbl
...
!			--------------------------------------------------------- 
     ...
     &			nStifEL,iEL,iOut)
			call ResultsAtDesiredSteps_LIN(SLamdam,THETAL,A(ndelUiEL),		! 3DLinear
!     &			nStifS,nQc,nDof,Radius,Conv,sign,inut,nDis,iStep,1,
     &			nStifEL,nQc,nDof,Radius,Conv,sign,inut,nDis,iStep,1,
     &			iOut,IoutDef,A(nEndw),A(m1),iEL)
...
!			call Quad_QQ_STIF(A(nxmO),A(m1),nQc,iEL,
			call Quad_QQ_STIF(A(ndelUiEL),A(m1),nQc,iEL,
     &			NelType,Nint,
