	Subroutine Form_Residual_External_Static(RExt,SKLM,TMat,
     &							nDof,nStifEL,Iout)
!	----------------------------------------------------------------------------- S Matrix
...
!	Outputs 
!		RExt	= External Body Force Only Residual Vector: RExt(nStifEL) at Integration Pt.
!	------------------------------------------------------------
...
!	------------------------------------------------------ Residual Internal
	RExt		= MATMUL(TRANSPOSE(TMat),BF) ! will add when BF is nonzero *******
!---------------------------------------------------------- Write:
	iPrt = 0
	call DumpRealArray(RExt,nStifEL,"Res_Ext_STAT",iPrt,iOut)
!----------------------------------------------------------
	return
!
	end
