...
!     &						Stif,BFors,StifK,nStif,Iout,Trace)
	Subroutine Quad_QQ_STIF_DYNAMIC(RoCOld,CuCOld,RoSav,CKSav,
     &						Qd,Qv,Qa,
     &						Nel,DMat,B,T,
     &						Stif,BFors,StifK,nStif,Iout,Trace)
!
...
!
	Real(kind=8)	dis       ,vel       ,acc                   
	Dimension		dis(nStif),vel(nStif),acc(nStif)
...
!
	Real(kind=8)	XM           ,GY           ,CF           ,DT        
	Dimension		XM(nDof,nDof),GY(nDof,nDof),CF(nDof,nDof),DT(nDof)
...
!		===============
		...
		if(iPrt == 1) then
			call DumpStringVar("Beg:Quad_QQ_STIF",iPrt,iOut)
...
!
		Wt = GaussPtVal(iGau,Nint1,2)*GaussPtVal(jGau,Nint2,2)
     &						 		 *RutAhat
		iPrt = 0
		if((iStep == 1).AND.(iNut == 2)) Then
			iPrt = 0
     ...
     &			(GaussPtVal(jGau,Nint1,2),"Gaussj",iPrt,iOut)
		...
		iPrt = 0
!		===============================================
		Stif = Wt*(MATMUL(TRANSPOSE(B),MATMUL(EDG,B)))
!		===============================================
!		------------------------------------------------------------- Load Vector: Residual:Gint-GExtStatic
!																evaluated at deltim = n + (1/2)
		call Form_Residual_Internal(BVec,FMns,BMat,
     &							nCMat,nDof2,nStifEL,Iout)
!
		BFors = BFors + Wt*BVec
!----------------------------------------------------------------------- if Static Skip
		if(cProbTyp == "STATIC ") then
		! ONLY External Body Force Needed because of arc length constraint treatment
			call Form_Residual_External_Static(BVec,SKLM,TMat,
     &											nDof,nStifEL,Iout)
			BFors = BFors - Wt*BVec
!			------------------------------------------------------------- Form: Last Column of
			StifK = StifK + Wt*SKLM     !stiffness: KLamda Body Force part
			go to 60
		endif
!----------------------------------------------------------------------- DYNAMIC PART
		call Split_Vel_RotV_DYNAMIC(Qv,nStif,T,Iout)
		call Split_Acc_RotA_DYNAMIC(Qa,nStif,T,Iout)
		call Form_AngularVEL_Vector(W,RotV,AVel,nC,Iout)
		call Form_AngularACC_Vector(W,WVel,RotV,RotA,AAcc,nC,Iout)
!		------------------------------------------------------------- Form & Add:T_trans_XM_T * (accmul)
!		============================================ XM(nDof,nDof):MASS Matrix
		call Form_Mass_Matrix(W,XM,nC,ndof,Iout)
!
!		Add to stiffness matrix the MASS matrix
!
!		===============================================
		Stif = Stif + AccMul*Wt*(MATMUL(TRANSPOSE(T),MATMUL(XM,T)))
!		===============================================
!		------------------------------------------------------------- Form & Add: Ttran_GY_T * (velmul)
!		================================================ GY(nDof,nDof):GYROSCOPIC Matrix
		call Form_Gyro_Matrix(W,WVel,RotV,GY,nC,ndof,Iout)
!
!		Add to stiffness matrix the GYROSCOPIC matrix
!
!		===============================================
		Stif = Stif + VelMul*Wt*(MATMUL(TRANSPOSE(T),MATMUL(GY,T)))
!		===============================================
!		------------------------------------------------------------- Form & Add: Ttran_CF_T 
!		================================================ CF(nDof,nDof):CENTRIFUGAL Matrix
		call Form_Centri_Matrix(W,WVel,WAcc,RR,RotV,RotA,
     &							RVel,RAcc,CF,nC,ndof,Iout)
!
!		Add to stiffness matrix the CENTRIFUGAL matrix
!
!		===============================================
		Stif = Stif + Wt*(MATMUL(TRANSPOSE(T),MATMUL(CF,T)))
!		===============================================
!		------------------------------------------------------------- Load Vector: Residual:Giner
!		call Residual(BVec,    )
!
...
!
	return
!
 1010 format("Full Stiffness Matrix:Stif(",I5,",",I5,")"/)
