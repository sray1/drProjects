	Subroutine Form_EtranDEplusGMatrix(DMat,EMat,
     &							TMat,BMat,EDG,Iout)
...
!		EFO(18)			= EtransposeF0 Vector at integration point R				
...
!	-------------------------------
	...
	if(iprt == 1) then
		ctrRot = " "
		trR = AMat(1,1) + AMat(2,2) + AMat(3,3)
		if(trR.LE.-0.99) then
			ctrRot = "  ***** Almost Singular ***** "
!	                  123456789012345678901234567890
			write(Iout,1100) iGau,jGau,trR,ctrRot
		endif
	endif
	iPrt = 0
!	-------------------------------
...
!
 1000	format(A,I5,A)
 1005 format("EMat(12,18)"/(I3,18(f5.2,1X)))
 1020 format("BVEC Vector(",I2,")"/(6(f13.7,1X)))
 1030 format("SKLM Vector(",I2,")"/(6(f13.7,1X)))
 1040 format(2X, "EDG Matrix(18,18)"/(I3,18(f10.0,1X)))
 1050 format(2X, "G Matrix(18,18)"  /(I3,18(f8.0,1X)))
 1100 format(10X,"iGau = ",I2,5X,"jGau = ",I2,5X,
     &			"Rotation Matrix Trace = ",f10.7,2x,A30)
	end
