	Subroutine DeformOut_MatLab_Hem(XLF,Qd,Qc,Conv,sign,iter,
...
!	//////////////////////--------------------------------- Computed at control node 0,0
	if(nEx == 10.AND.nElem == 4) then
		...
		elseif(Nel == 2) then 
			...
			EndwSav(1,iStep+1) = XLF
		endif
	else
		call CalcDis_at_a_Pt(Dis,Qd,nQd,nDim,nQd1,nQd2,1.D0,0.D0)
		iPlotCountEnd  = iPlotCountEnd + 1
		EndwSav(2,iStep+1) = ABS(Dis(3))				! wEnd
		call CalcDis_at_a_Pt(Dis,Qd,nQd,nDim,nQd1,nQd2,0.D0,0.D0)
		EndwSav(3,iStep+1) = ABS(Dis(3))				! wEnd
!		EndwSav(1,iStep+1) = 2.D0*XLF
