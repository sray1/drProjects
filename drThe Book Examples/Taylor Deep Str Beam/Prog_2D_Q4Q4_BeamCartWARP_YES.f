...
!------------------------------------------------------------------- Heading
	write(iOut,*)
     ...
     &	'==============='
	write(iOut,*)
     &	'   T A Y L O R  C U R V E D  B E A M:  Chapter 2  '
	write(iOut,*)
     &	'=========================================================',
     &	'==============='
!----------------------------		
...
!		nDegr	= 4	! r-direction disp element degree	Cubic	
!		nDegs	= 4	! s-direction disp element degree	Cubic	
!
		nmDof	= nEnd
		nDof	= nmDof		+ nDis*nSzInt
		nmSup	= nDof		+ nDis*nSzDbl
		nEnd	= nmSup		+ nSup*nSzInt
		call Cubic_Data(A(nDof),A(nmDof),nDis,A(nmSup),nSup,
     &					Nint,nDim,nDegR,nDegS,nStifS,iOut)
!
	elseif (cElemTyp == "44") then
		nDis	= 5	! Specified Displacement DOFs
		nSup	= 6 ! Supported DOFs	
!		nDegr	= 5	! r-direction disp element degree	Quartic
!		nDegs	= 5	! s-direction disp element degree	Quartic	

		nmDof	= nEnd
		nDof	= nmDof		+ nDis*nSzInt
		nmSup	= nDof		+ nDis*nSzDbl
		nEnd	= nmSup		+ nSup*nSzInt
		call Quartic_Data(A(nDof),A(nmDof),nDis,A(nmSup),nSup,
     &						Nint,nDim,nDegR,nDegS,nStifS,iOut)
!
	elseif (cElemTyp == "55") then
		nDis	= 6	! Specified Displacement DOFs
		nSup	= 7 ! Supported DOFs	
!		nDegr	= 6	! r-direction disp element degree	Quartic
!		nDegs	= 6	! s-direction disp element degree	Quartic	

		nmDof	= nEnd
		nDof	= nmDof		+ nDis*nSzInt
		nmSup	= nDof		+ nDis*nSzDbl
		nEnd	= nmSup		+ nSup*nSzInt
		call Quintic_Data(A(nDof),A(nmDof),nDis,A(nmSup),nSup,
     &						Nint,nDim,nDegR,nDegS,nStifS,iOut)
...
!-------------------------------------------------------------- Get Stiffness Matrix
	call Quad_CC_STIF(A(n1),nDim,nQc1,nQc2,Nel,NelType,Nint,
     &				E,PR,Thk,R_In,nDegR,nDegS,
...
!	----------------------------------------- Get Load Vector from displacement at End 
		call LoadfromDis(A(n2),nStifS,nStifS,A(nDof),A(nmDof),
...
!	--------------------------------------------------------- YES warp
	call Extract_NonSingularStif(A(n2),nStifC,nStifC,
