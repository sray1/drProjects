...
!
	...
	real(kind = 8)	Dof(nDis),F(nRem),Ftem(nRem)
	integer			nSsI,nSsJ
...
!
		...
		iPrt = 0
		if(iPrt == 1) then
			write(iOut,1020) (Ftem(jj),jj = 1,nRem)
			write(iOut,1010) (F(jj),jj = 1,nRem)
!			Pause
		endif
!
		Ftem = 0.D0
