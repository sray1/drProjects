...
!
 	FeDis(1,i+nStart) = sumU*P2			 
 	FeDis(2,i+nStart) = sumV*P2			 
 	FeDis(3,i+nStart) = sumW*P2
 	FeDis(4,i+nStart) = sum1*P2			 
 	FeDis(5,i+nStart) = sum2*P2			 
 	FeDis(6,i+nStart) = sum3*P2
!--------------------------------------------------- Cartesian-->Curvilinear
	call QQ_TMB_CuLn(Qc,nQc,nQc,ri,H1D,Det,Curvature,Torsion,Axes,
     &							dxbardr,Nel,Iout,0)
!
	sumTX = Axes(1,1)*sumU+Axes(1,2)*sumV+Axes(1,3)*sumW
	sumTY = Axes(2,1)*sumU+Axes(2,2)*sumV+Axes(2,3)*sumW
	sumTZ = Axes(3,1)*sumU+Axes(3,2)*sumV+Axes(3,3)*sumW
!
	sumRX = Axes(1,1)*sum1+Axes(1,2)*sum2+Axes(1,3)*sum3
	sumRY = Axes(2,1)*sum1+Axes(2,2)*sum2+Axes(2,3)*sum3
	sumRZ = Axes(3,1)*sum1+Axes(3,2)*sum2+Axes(3,3)*sum3
!
