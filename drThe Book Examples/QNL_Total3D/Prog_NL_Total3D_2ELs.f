	Program NonLinear_Total3D_Snap_RIKS
!------------------------- Modules
...
!
	iDebug	= 0
	iPlotCount  = 0	! Scigraph
	iPlotCountDef  = 0	! MatLab
	iPlotCountEnd  = 1	! MatLab
...
!	--------------------------------------------- Endw Plot
	...
	call PlotDataEndw(A(nEndwSav),iPlotCountDef-1,IoutEnd)	!ignore last one that closes figure
!
	write(IoutDef,1040) float(iPlotCountDef+1),float(nDis)
	write(IoutEnd,1040) float(iPlotCountEnd+1),float(iPlotCountDef-1)
!	------------------------------------------------------------- MatLab Plot End
	iPlot = 0
	if(iPlot == 1) Then
		iPltType = 2
