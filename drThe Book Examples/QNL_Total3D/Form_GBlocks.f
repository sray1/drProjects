	Subroutine FormGBlocks(HD,nG,FM,nF,C0,nC,W,R,AA,CK,
     &					G12,G22,G23,G24,Iout)
!	----------------------------------------------------------------------------- 
!	Inputs
...
!
	Real(kind=8)	HD    ,FM    ,C0                       
	Dimension		HD(nG),FM(nF),C0(nC)
	Real(kind=8)	G12       ,G22       ,G23       ,G24                   
	Dimension		G12(nC,nC),G22(nC,nC),G23(nC,nC),G24(nC,nC)
	Real(kind=8)	Di    ,Dp    ,Ro    ,Rp    ,Fo    ,XM                  
	Dimension		Di(nC),Dp(nC),Ro(nC),Rp(nC),Fo(nC),XM(nC)
	Real(kind=8)	CK    ,AA    ,BB    ,CC    ,Hat1                     
	Dimension		CK(nC),AA(nC),BB(nC),CC(nC),Hat1(3)
...
!	
	call Extract_Vector(FM,nF,Fo,nC,1,Iout)	!Force 
	call Extract_Vector(FM,nF,XM,nC,4,Iout)	!Moment 
!-----------------------------------------------------------------------------------------G22Block
!	------------------------------------------------------------------- Translational
...
!	--------------------------------------- .5*XMK + c2(theta dot c)I
	XMK = 0.5D0*XMK
	XMK(1,1) = XMK(1,1) + C2TdotB
	XMK(2,2) = XMK(2,2) + C2TdotB
	XMK(3,3) = XMK(3,3) + C2TdotB
!	---------------------------------------------------------------AlfaCxRot + RotxAlfaC
