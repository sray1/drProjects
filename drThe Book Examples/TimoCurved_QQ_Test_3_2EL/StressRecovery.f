...
!------------------------------------------------	FeShe:Alternate: Integrated Axial	
	elseif ( 1 == 1 ) Then						 
...
!	//////////////////////--------------------------------- Exact
	...
	if(Nel == 2) Then
		...
		si = delsi*(i-1)
		if(i.LE.nPts-1) Then
			FeMom(2,i) =  P*(1.D0 - DCOS(si) + DSIN(si))/2.D0	! 
		else
			FeMom(2,i) = -P*(1.D0 + DCOS(si) - DSIN(si))/2.D0	! 
		endif
		FeAxi(2,i) = -P*DSIN(si-pi/4.D0)/DSQRT(2.D0)/Radius
		FeShe(2,i) =  P*DCOS(si-pi/4.D0)/DSQRT(2.D0)/Radius
  100     continue 
	endif		 
!
	return
