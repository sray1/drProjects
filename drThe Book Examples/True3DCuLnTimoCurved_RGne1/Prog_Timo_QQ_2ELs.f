	Program TRUE_3DCurvilinear_Timo
!
	USE DFPORT
...
!
	...
	character(20)   RootG
	dimension		RootG(2)
	DATA			RootG		/"Assumed 1","NOT Asuumed 1"/ 
!---------------------------
!		
	iRootG = 2
!===============
...
!	------------------------------
	if(nQc == 5) Then
		write(iOut,1000) RootG(iRootG),ElDeg(1),nDof,nElem,nStifEL
	else if(nQc == 6) Then
		write(iOut,1000) RootG(iRootG),ElDeg(2),nDof,nElem,nStifEL
	endif
!	------------------------
...
!-------------------------------------------------
	Nel = 1
	call BZ_Coords_EL1(A(n1a),3,nQc,RADIUS,nElem,iOut)
!-------------------------------------|------|--------
!	Get Stiffness Matrix: Ist Elem 
!
	if(iRootG.EQ.1) Then
		call Quad_QQ_STIF(A(n1a),nQc,Nel,NelType,Nint,E,G,PR,
     &			Depth,Width,Radius,A(n2a),nStifEL,Iout,Trace)
	elseif(iRootG.NE.1) Then
		call Quad_QQ_STIF_ne1(A(nXP),A(n1a),nQc,Nel,NelType,Nint,
     &		E,G,PR,Depth,Width,Radius,A(n2a),nStifEL,Iout,Trace)
	endif 
!
!	Assemble Ist Elem
!
...
!-------------------------------------------------
	if(nElem == 2) Then
		Nel = 2
		call BZ_Coords_EL2(A(n1b),3,nQc,RADIUS,iOut)
!	-------------------------------------|-----|--------
		if(iRootG.EQ.1) Then
			call Quad_QQ_STIF(A(n1b),nQc,Nel,NelType,Nint,E,G,
     &			PR,Depth,Width,Radius,A(n2b),nStifEL,Iout,Trace)
		elseif(iRootG.NE.1) Then
			call Quad_QQ_STIF_ne1(A(nXP),A(n1b),nQc,Nel,NelType,Nint,
     &			E,G,PR,Depth,Width,Radius,A(n2b),nStifEL,Iout,Trace)
		endif
!
!		Assemble 2nd Elem
!
...
!
     ...
     &			nDis,(nDis+1)/2,nDis+1,1,iOut)
			call StressRecovery_2ELModel(A(nXP),A(n1a),nQc,A(n2a),nStifEL,
     &			E,PR,RADIUS,DEPTH,WIDTH,P,A(n4),nStifEL,
     &		A(nF1),A(nF2),A(nF3),A(nM1),A(nM2),A(nM3),A(nMSA),
     &		nStr,nStr,nStr+1,1,iOut,A(nCT))
		else
			call Compare_DisRot_2ELModel(A(n1a),nQc,A(n4),nStifEL,P,
     &			A(nU),A(nV),A(nW),A(nRx),A(nRy),A(nRz),A(nMDR),
     &			nDis,(nDis+1)/2,nDis+1,1,iOut)
!	---------------------------------------------------------------
			call StressRecovery_2ELModel(A(nXP),A(n1a),nQc,A(n2a),
     &			nStifEL,E,PR,RADIUS,DEPTH,WIDTH,P,A(n4),nStifEL,
     &		A(nF1),A(nF2),A(nF3),A(nM1),A(nM2),A(nM3),A(nMSA),
     &		nStr,(nStr+1)/2,nStr+1,1,iOut,A(nCT))
		endif
!	---------------------------------------------------------------
...
!
		if(nElem == 2) Then
			...
			call Compare_DisRot_2ELModel(A(n1b),nQc,A(n4),nStifEL,P,
     &			A(nU),A(nV),A(nW),A(nRx),A(nRy),A(nRz),A(nMDR),
     &			nDis,(nDis+1)/2,nDis+1,2,iOut)
!	---------------------------------------------------------------
			call StressRecovery_2ELModel(A(nXP),A(n1b),nQc,A(n2b),
     &			nStifEL,E,PR,RADIUS,DEPTH,WIDTH,P,A(n4),nStifEL,
     &		A(nF1),A(nF2),A(nF3),A(nM1),A(nM2),A(nM3),A(nMSA),
     &		nStr,(nStr+1)/2,nStr+1,2,iOut,A(nCT))
!	---------------------------------------------------------------
			call Compare_Castig_2ELModel(E,PR,RADIUS,DEPTH,WIDTH,P,!QuickDirty
     &							A(n4),nStifEL,2,iOut)
		endif
!--------------------------------------------------------------------
	end if
...
!
	Stop 'OK'
 1000 Format(/ 5x,"TRUE 3D CANTILEVERED: CURVILINEAR"/
     &	2x,"Metric Determinant         = ",A20/
