...
!
	...
	integer			n1a,n1b,n2
	real(kind = 8)	Disp   ,DepData    ,wc,uc,tc
	dimension		Disp(2),DepData(8)
!
	DATA DepData /1.00,0.50,0.30,0.20,0.10,0.05,0.01,0.001/
	DATA iTotDep /8/
!---------------------------
!		
!	
...
!-------------------------- Load
	P		= -1.D0		! Tip Radial at 90 degree							
!-------------------------- Geometry
	RADIUS	= 10000.D0	! Radius
!--------------------------------------
!	XLENGTH = 10.D0/DSQRT(12.D0)	! to accomodate Slenderness Ratio of
!	do 100 idep = 1,iTotDep
!	DEPTH	= DepData(iDep)	! Depth
!--------------------------------------  a Straight Beam
	XLENGTH = 10.D0
	DEPTH	= .01D0	! Depth
	WIDTH	= 1.D0		! Width
!-----------------------------------------------------------------
	write(iOut,1010) nStifS,iGeom,iCont,nC,nStifC,nSup,nStifN,Nint,
     &				Neltype
	write(iOut,1020) E,PR,XLENGTH,RADIUS,DEPTH,WIDTH,P
!-------------------------------------------------------
!	Array sizes
!
	nSzInt	= 4 !bytes
...
!-------------------------------------------------
	Nel = 1
	call BZ_Coords_EL1(A(n1a),2,nQc,RADIUS,XLENGTH,iOut)
!-------------------------------------|------|--------
...
!	---------------------------------------------------------------
     ...
     &		E,PR,XLENGTH,RADIUS,DEPTH,WIDTH,P,A(n4),nStifEL,
     &		A(nM),A(nS),A(nA),A(nMSA),nStr,nStr,nStr,1,iOut)
		call Compare_EulerBernoulli(P,xLENGTH,E,PR,RADIUS,DEPTH,WIDTH,
     &											A(n4),nStifEL,iOut)
!--------------------------------------------------------------------
	end if
!
...
!
	Stop 'OK'
 1000 Format(/ 5x,"TestCase IV: 1 Element"/
 ...
 1020 Format(/ 5x,"Real Controls"/
     &	2x,"Elasticity Modulus         = ",F20.5/
     &	2x,"Poisson's Ratio            = ",F20.7/
     &	2x,"Length                     = ",F20.7/
     &	2x,"Radius                     = ",F20.7/
     &	2x,"Depth                      = ",F20.7/
     &	2x,"Width                      = ",F20.7/
     &	2x,"Tip Radial Load            = ",F20.7/
     &	)		
	end 
!	  			
