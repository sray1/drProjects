	Subroutine Stif_Element(W,Qc,nDim,nQc1,nQc2,Nel,NelType,Nint,
     &				E,PR,Thk,R_InSide,nDegR,nDegS,nElems,
     &				Depth,Stif,nStif,Iout,Trace,bRational)
!
...
!
	Real(kind=8)	Qc(nDim,nQc1,nQc2),W(nDim,nQc1,nQc2),E,PR,Thk
...
!	else
!		Ri = Rk(lr,Nint)
		Ri = GaussPtVal(lr,Nint,1)	!Rk(lr,Nint)
!	endif
	do 60 ls = 1,Nint
!	if (Nint == 16) Then
!		Si = GLO(ls,1)
!	else		 
!		Si = Rk(ls,Nint) 
		Si = GaussPtVal(ls,Nint,1)  !Rk(ls,Nint) 
!	endif
!
...
!----------------------------------------------------------- Strain Displacement
	if(bRational) then
		call CC_StrainDispRAT(W,Qc,nDim,nQc1,nQc2,nQc,nQd,Ri,Si,B,nB,
     &		R_InSide,Depth,nDegR,nDegS,Det,Nel,nElems,NelType,Iout,
     &        bRational)
	else
		call CC_StrainDisCart(Qc,nDim,nQc1,nQc2,nQc,nQd,Ri,Si,B,nB,
     &		R_InSide,Depth,nDegR,nDegS,Det,Nel,nElems,NelType,Iout)
	endif

!	call CC_StrainDispMat(Qc,nDim,nQc1,nQc2,nQc,nQd,Ri,Si,B,nB,
!     &	R_InSide,Depth,nDegR,nDegS,Det,Nel,nElems,NelType,Iout)
!----------------------------------------------------------- Quartic_Quartic
!	write(*,1020) ri,si
!	do 10 i = 1,3
...
!
!	if (Nint == 16) Then
!		Wt = GLO(lr,2)*GLO(ls,2)*Det
!	else
!		Wt = Ak(lr,Nint)*Ak(ls,Nint)*Det
		Wt = GaussPtVal(lr,Nint,2)*GaussPtVal(ls,Nint,2)*Det
!	end if
	do 50 j = 1,nStif
...
!
	iPrt = 0
	if(iPrt == 1) then
		write(iOut,1030) (j,Add(j),j = 1,nStif)
	endif
...
!
 ...
 1030 format("Column Sums of Full Stiffness Matrix"/
     &         4(I5,2x,F15.5,5x) )
	return
	end   
