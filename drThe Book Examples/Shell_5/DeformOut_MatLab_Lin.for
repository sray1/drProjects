	Subroutine DeformOut_MatLab_LIN(XLF,THETAL,Qd,nQd,nQc,nDof,
     &            	Rad,Conv,sign,iter,nPts,nStep,nPStep,iOut,iOutS,
     &				EndwSav,Qc,Nel)
	Implicit Real(kind=8) (a-h,o-z)
	logical SaveStep
!
	Real(kind=8)	Qc       ,Axes     ,geom   ,XI      
	Dimension		Qc(3,nQc),Axes(3,3),geom(3),XI(3,3)
...
!
   ...
   40		continue
		...
		w = geom(3) + sumTZ
!--------------------------------------------
		write(iOutS,1020) u,w,v
!--------------------------------------------
!--------------------------------------------------- Curvilinear-->Cartesian:END
!		ri = ri + delri
   50		continue
!=================================================== check: identity OK
!		XI = MATMUL(Axes,TRANSPOSE(Axes))
!=================================================== check end
		uEnd = u 
		vEnd = v
		wEnd = w
!		----------------------------------------- save for Plot
		...
		EndwSav(2,nStep+1) = uEnd	! Coordinates
		EndwSav(3,nStep+1) = vEnd
		EndwSav(4,nStep+1) = wEnd
!		EndwSav(2,nStep+1) = sumTX	! Tip Displacements
...
!
	...
	if(iPrt == 1) Then
		if(Nel == 1) Then
			...
			write(iOut,1015)       XLF,uEnd,vEnd,wEnd,iter,Conv,sign
		endif
	endif
!
	return
!
 ...
 1015 format(3x,1x,4(3x,e12.5),i5,e12.4,2x,a1)
