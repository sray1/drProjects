...
!------------------------------------------------------------------- Continuity
	if(nEx == 10.AND.nElem == 4) then
		call Set_IntegerControls_410(iDebug,iOut)
	else
		call Set_IntegerControls(iDebug,iOut)
	endif
!============================================================================== HEM PreProcess
	call Set_Files_Scratch()
!	call Hem_Preprocess(iOut)
!============================================================================== HEM PreProcess End
!------------------------------------------------------------------- Element Coords
	nEndW		= 1
	n1a			= nEndW
...
!
	call Get_ElementCoords(iConFEA,iOut)
!------------------------------------------------------------------- Material
...
!	--------------
	...
	do 10 iEL = 1,nElem
		...
		call Put_Reference_AllGauss(A(n1XXX),A(nTMat),A(nBMat),iEL)
   10 continue
!------------------------------------------------------------------- Constitutive Matrix
	nDMat		= nEndW
	nEndW		= nDMat 		+ nCMat*nCMat *nSzDbl
	call Form_ConstitutiveMatrix(A(nDMat),nCMat,iOut)
!=================================================================================== Phase I: END
	return
!
