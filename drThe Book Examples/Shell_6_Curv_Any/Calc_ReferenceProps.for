...
!	----------------------------------------------------------- Lines of curvatures
	FEG =F/E/G
	if (dabs(FEG).le.1.0D-3.or.xM==0.d0) then	! already lines of curvatures
		Rho1	= zero		! direction
		Rho2	= zero		! direction
		Cu1		= xL/E		! 1/R1
		Cu2		= xN/G		! 1/R2
...
!
		...
		TEGmFsq		= 2.D0*EGmFsq
		xLNmMsq		= xL*xN - xM*xM
		TFMmENpGL	= 2.D0*F*xM - ENpGL
		SQ		= DSQRT(TFMmENpGL*TFMmENpGL - 4.D0*EGmFsq*xLNmMsq)
		Cu1		= (-TFMmENpGL + SQ)/TEGmFsq 
		Cu2		= (-TFMmENpGL - SQ)/TEGmFsq 
!
		ThetaC	= DATAN(Rho1)
		sT		= DSIN(ThetaC)
		cT		= DCOS(ThetaC)
...
!
	...
	if(iPrt == 1) then
		...
		call DumpRealVar(ThetaD,"ThetaC in degree",iPrt,iOut)
