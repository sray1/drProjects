	Subroutine Quad_CC_STIF(Qc,nQc,Nel,NelType,Nint,E,PR,Thk,Depth,
     &						Stif,nStif,Iout,Trace)
!
...
!-------------------------------------------------- 
	...
	Dimension    Rk(6,6),Ak(6,6),D(2,2),B(2,8),Add(8),DB(4)
...
!	Stress-Strain Law
!
		XK		= 5.0D0/6.0D0					! Shear Factor
		Area	= Depth*Thk						! Area
		G		= 0.5D0*E/(1.0D0+PR)			! Shear Modulus
		XI		= Thk*Depth*Depth*Depth/12.0D0	! Inertia
		TH		= Thk							! thickness
!
	D		= 0.0D0	!All	
	D(1,1) = E*XI
...
!	Loop over Gauss Locations
	...
	Ri = Rk(lr,Nint) 
!
!	Get Strain Displacement Matrix: B(2,8)
!
	Call CC_StrainDispMat(Qc,nQc,Ri,B,8,Det,Nel,NelType,Iout)
!
!	Add to stiffness matrix
!
	Wt = Ak(lr,Nint)*Det*TH
	do 50 j = 1,8
	do 20 k = 1,2		! 
	DB(k) = 0.0
	do 20 l = 1,2		! 
...
!	Pause
  110	continue
	return
!
 1010 format("Full Stiffness Matrix:Stif(8,8)"/
     &        I5/(4(8(f10.6,1X)/)/))
	return
	end   		