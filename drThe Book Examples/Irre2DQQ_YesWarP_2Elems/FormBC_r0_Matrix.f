...
!
	...
	Integer DofR1,DofR2,Loc1(4),Loc2(4), nDof
	DATA DofR1  /11/			!To be removed by
	DATA Loc1  /1, 6 ,15,20/	! these 
	DATA DofR2  /36/			!To be removed by
	DATA Loc2  /25,30,39,44/	! these 
!	DATA nDof/2/
!	//////////////////////
...
!
	...
	do 30 i = 1,nStifS
		if(i == DofR1) Then
			T(i,Loc1(1)) = -1.0D0/6.0D0
			T(i,Loc1(2)) = -2.0D0/3.0D0
			T(i,Loc1(3)) = -2.0D0/3.0D0
			T(i,Loc1(4)) = -1.0D0/6.0D0
			iremoved = iremoved + 1
			go to 20
		else if(i == DofR2) Then
			T(i,Loc2(1)) = -1.0D0/6.0D0
			T(i,Loc2(2)) = -2.0D0/3.0D0
			T(i,Loc2(3)) = -2.0D0/3.0D0
			T(i,Loc2(4)) = -1.0D0/6.0D0
			iremoved = iremoved + 1
			go to 20
		end if
   10     continue
