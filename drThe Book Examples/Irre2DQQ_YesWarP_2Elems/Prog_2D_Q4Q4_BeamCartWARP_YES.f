...
!	nStifS	= 32  !	for Cubic_Cubic
	nStifS	= 50  !	for Quartic_Quartic
!	nCon	= 2	! YES WARP
	nCon	= 0	! NO WARP: Fixed End
	nStifC	= nStifS - nCon
!	nSup	= 8  !	for Cubic_Cubic
	nSup	= 10 !	for Quartic_Quartic
	nStifN	= nStifC - nSup
!	-----------------------	
	Nel		= 1
	Nint	= 4	! Gauss Order "CRUCIAL" for Very High (R/h)
!	Nint	= 16		! Gauss Order
...
!-------------------------------------------------------------- QuarticxQuartic
	call Quad_CC_STIF(A(n1),25,Nel,NelType,Nint,E,PR,Thk,Rad,
     &					Depth,A(n2),nStifS,Iout,Trace)
!
...
!-------------------------------------- Quartic
	Dimension		Drop(10)	
	DATA Drop / 1,6,11,16,21,26,31,36,41,46/
	DATA nDrop/10/
!
	ii = 0
	jj = 0
