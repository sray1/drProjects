...
!
!	Try Coerced Isoparametric Quadrilateral_Quart_Quart Element
!
!--------------------------------------------------------------------------------
...
!	nStifActual = 64 - 8 (tie dofs between 2 Elements) = 56
	...
	nOrderS	= 5
	nQc		= nOrderR*nOrderS
	nStifEL	= 2*nQc						! x and y
	nStifS	= 2*nStifEL - 2*nOrderR		! Singular ( x & y Jn. Dofs) 
!------------------------------------------------------------------- Continuity
	iGeom	= 0 
!	------------
...
!-------------------------------------------------
	...
	!-----------------------------------
		nC		= 0
		nStifC  = nStifS - nC	! BC at r=0 replaced by direct support
...
!---------------------------------------------------------------- Support Cond
	nSup	= 5
	nStifN  = nStifC - nSup	! size NonSingular: yy = xx - 5( 2 supX + 1 SupY
!															@ r=0,s=0 & s= 1
!															+1 SupX + 1 SupY
!															@ r=0,s=0.5
!-----------------------------------------------------------------------------
...
!-------------------------- 
	xLy		= 12.0D0
!-----------------------------------------------------------------
	write(iOut,1010) nOrderR,nOrderS,nStifS,iGeom,iCont,nC,nStifC,
     &					nSup,nStifN,Nint,Neltype
...
!-------------------------------------------------------
	...
	nSzDbl	= 8 !bytes
	n1a		= 1								! Qc1(2,25)
	n1b		= n1a + 2*nQc*nSzDbl			! Qc2(2,25)
	n2		= n1b + 2*nQc*nSzDbl			! S(50,50) Singular / 2ELs(n2)
	n2b		= n2  + nStifS*nStifS*nSzDbl	! S(50,50) Singular / 1ELs(n2b)
...
!
	Nel = 1
	call Quad_CC_STIF4(A(n1a),nQc,Nel,NelType,Nint,E,PR,xLx,xUx,xLy,
     &								Thk,A(n2b),nStifEL,Iout,Trace)
!
!	Assemble Ist Elem
!
	n2c		= nEnd
...
!                                     ^     ^
!-------------------------------------|-----|--------
	call Quad_CC_STIF4(A(n1b),nQc,Nel,NelType,Nint,E,PR,xUx,xLx,xLy,
     &								Thk,A(n2b),nStifEL,Iout,Trace)
!
!	Assemble 2nd Elem
!
...
!--------------------------------------------------------------------
!	C0_Condition: NOT NEEDED because Node 11 is right at r=0;s=0.5
!--------------------------------------------------------------------
	if (iCont == 0) then
!
...
!
 ...
 1010 Format(/ 5x,"Integer Controls"/
     &	2x,"Order_R = Deg_R + 1		   = ",I5/
     &	2x,"Order_S = Deg_S + 1		   = ",I5/
