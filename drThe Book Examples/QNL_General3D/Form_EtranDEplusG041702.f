	Subroutine Form_EtranDEplusGMatrix(Axes6,
     &				RoOld,Hd,nStifEL,DMat,C0,RK,CK,
...
!
	Real(kind=8)	FMc
	Dimension		FMc(nDof)
	Real(kind=8)	Axes6           ,DMatC 
	Dimension		Axes6(nDof,nDof),DMatC(nDof,nDof)
...
!--------------------------------------------------------------------- ETransDE+G (12x12)
!	DMATC		= MATMUL(Axes6,MATMUL(DMAT,TRANSPOSE(Axes6)) )
!	EDG			= MATMUL(MATMUL(TRANSPOSE(EMat),DMatC),EMat) + G
...
!	FMc			= MATMUL(TRANSPOSE(Axes6),FMn)	! FMc = Cartesian
!	EFO			= MATMUL(TRANSPOSE(EMat),FMc)	!EFO(12x1) = ETrans(12x6) FMc(6x1)
