	Subroutine FormThetaAlfaTrans_Matrix(t,p,S,nS,c1,c2,c1b,c2b,Iout)
!	----------------------------------------------------------------------------- S Matrix
!	t		= theta column vector
!	p		= c = (M cross (ko + k) column vector
!
...
!	c2Bar	= (c1 - 3*c2)/tL2
!
!	theta_AlfTrans	= (theta x pTrans)(-c2*I - c1Bar*thetaMatrix + c2Bar*thetaMatrix^^2)
!	----------------------------------------------------------------------------- 
!	Inputs
...
!	----------------------------------------------------------------------------- R Matrix
	S = zero
!
	S = S - c2*ttp - c1b*tmtpt + c2b*t2tpt
!---------------------------------------------------------- Write:
	iPrt = 0
	if (iPrt == 1) Then
...
!
 1000	format(A,I5,A)
 1010 format("Theta_AlfaTrans Matrix(3,3)"/( (I5,3(f20.7,1X))  ))
	end
