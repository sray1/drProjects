!	Subroutine FormEMatrix(AMat,C0,nC,E,nE,W,R,AA,CK,nG,Iout)
	Subroutine Opt1_FormEMatrix_2(EMat,nE,nG,nC,iDir,Iout)
!	----------------------------------------------------------------------------- 
!	Inputs
...
!	--------------------------- Corrector for dir 2 based on Engineering or Virtual For/Mom
	E11 = MATMUL(MATMUL(B2b,E11),A2b)
	E12 = MATMUL(MATMUL(B2b,E12),A2b)
	E13 = MATMUL(MATMUL(B2b,E13),A2b)
!	--------------------------------- bending: 
	E22 = MATMUL(MATMUL(B2k,E22),A2k)
	E24 = MATMUL(MATMUL(B2k,E24),A2k)
!---------------------------------------------------------- Write:
	iPrt = 0
	if (iPrt == 1) Then
