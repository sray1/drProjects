!	Subroutine FormEMatrix(AMat,C0,nC,E,nE,W,R,AA,CK,nG,Iout)
	Subroutine Opt1_FormEMatrix_1(EMat,nE,nG,nC,iDir,Iout)
!	----------------------------------------------------------------------------- 
!	Inputs
...
!	--------------------------- Corrector for dir 1 based on Engineering or Virtual For/Mom
	E11 = MATMUL(MATMUL(B1b,E11),A1b)
	E12 = MATMUL(MATMUL(B1b,E12),A1b)
	E13 = MATMUL(MATMUL(B1b,E13),A1b)
!	--------------------------------- bending: 
	E22 = MATMUL(MATMUL(B1k,E22),A1k)
	E24 = MATMUL(MATMUL(B1k,E24),A1k)
!---------------------------------------------------------- Write:
	iPrt = 0
	if (iPrt == 1) Then
