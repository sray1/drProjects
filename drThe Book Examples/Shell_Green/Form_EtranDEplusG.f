...
!============================================
	...
	include 'LogParams.h'
	include 'Material.h'	! for Condensed Constitutive Matrix(8,8)
	include 'EBlocks.h'
!==============================================
...
!	-------------------------------
	call FormR_Matrix(Ro,Rp,RMat1,nC,c1,c2,c1b,c2b,Iout) !R matrix + New Notation :- X Matrix
	RMat = RMat1
!	===============================================
!	call FormEBlocks(nC,E11_1,E12_1,E13_1,E32_1,E35_1,1,Iout)
!	===============================================
	CK1		= CK
	AA1		= AA
	AMat1	= AMat		
...
!	-------------------------------
	...
	RMat = RMat2
!	===============================================
!	call FormEBlocks(nC,E11_2,E12_2,E13_2,E32_2,E35_2,2,Iout)
!	=============================================== 
	CK2		= CK
	AA2		= AA
	AMat2	= AMat		
...
!																	and (n-Mn-Qn coords) for GG Mat
!	call FormForMom(DMat,rE,nCMat,AMat1,AMat2,nC,nDof,FMt1,FMt2,
!     &											FMn,FMns,Iout)
!--------------------------------------------------------------------- E( 8x18)
	call FormEMatrix_Green(EMat,11,nDof2,nDim,Iout)
!--------------------------------------------------------------------- GG(18x18)
!	call FormGMatrix(GG,nDof2,nDim,nDof,Iout)
!
	GG = zero
!--------------------------------------------------------------------- ETransDE+GG (18x18)
	EDG			= MATMUL(MATMUL(TRANSPOSE(EMat),CD),EMat) + GG
!	-------------------------------------------------------- Debug 
	iPrt = 0
	if(iprt == 1) then
...
!	if((iStep == 1).AND.(iNut == 2)) Then
		...
		write(Iout,1050) (i,(  GG(i,j),j = 1,nDof2),i=1,nDof2)
...
!
	if (bPen) then
		EFO			= pen*(rE(2)-rE(7))*FMnC	!EFO(18x1) = pen*ECTrans(18x12)*(beta12-beta21)
		BVEC		= BVEC + MATMUL(TRANSPOSE(BMat),EFO)
	endif
...
!		-------------------------------------------------------- Debug 
		if((iStep == 9999).AND.(iNut == 0)) Then
			...
			call DumpRealMatrix(GG,nDof2,nDof2,"GMat",iPrt,iOut)
...
!
 ...
 1050 format(2X, "GG Matrix(18,18)"  /(I3,18(f7.2,1X)))
	end
