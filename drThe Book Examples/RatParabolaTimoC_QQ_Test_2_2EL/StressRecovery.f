...
!
	Real(kind=8)	rh,si,delsi,AM,ri,delri,RadOfCur
	Real(kind=8)	E,R,Depth,Width,P2
	...
	Real(kind=8)	H1D     ,Reac   ,Axes     ,dxbardr      	 
...
!
	...
	call RatGeometryProps(Qx,Qy,Wt,nQc,ri,Det,RadOfCur,Axes,
     &							Nel,Iout)
...
!------------------------------------------------- FeAxi
	if ( 1 == 2) Then
		call Bernstein(B2,nQc,ri,iOut)
!		------------------------------
		sumX = 0.D0
		sumZ = 0.D0
		do 25 jj= 1,nQc
		sumX = sumX + B2(jj)*Qc(1,jj)	! x-coord
   25		sumZ = sumZ + B2(jj)*Qc(2,jj)	! z-coord
!		--------------
		Fac = 0.D0
		if(sumZ .NE. 0.D0) Then
			Fac = sumX/sumZ
		endif		
		FeStr(3,i+nStart) = - Reac(2) + 
     &						Fac * ( FeStr(2,i+nStart) - Reac(1) )  			 
		if(Nel == 1) Then
